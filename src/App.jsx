...
async function sheetCall(action, payload={}) {
  try {
    const r = await fetch(getScriptUrl(),{method:"POST",headers:{"Content-Type":"text/plain"},body:JSON.stringify({action,...payload})});
    return await r.json();
  } catch { return null; }
}
...
function SliderField({label,min,max,step=0.1,value,onChange,optimal,unit="",warnAbove,warnBelow}) {
...
function SetupScreen({ onDone }) {
  ...
  const [url,    setUrl]    = useState(getCompany().scriptUrl||"");
  ...
  const save = async () => {
    ...
    setSaving(true);
    saveCompany({ name: name.trim(), scriptUrl: url.trim()||FIXED_SCRIPT_URL });
    if (url.trim()) localStorage.setItem("galileo_script_url", url.trim());
    ...
    <div dir="rtl" style={{minHeight:"100vh",background:`linear-gradient(145deg,#0d47a1,#1565c0,#1976d2)`,fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{color:"rgba(255,255,255,0.65)",fontSize:13,margin:0}}>הגדר את שם החברה שלך</p>
        </div>
        ...
        <div style={{background:"#fff",borderRadius:24,padding:24,boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
          ...
          <div style={{marginBottom:6}}>
            <label style={{fontSize:12,fontWeight:700,color:"#90a4ae",display:"block",marginBottom:6}}>Apps Script URL <span style={{fontWeight:400,opacity:0.7}}>(אופציונלי)</span></label>
            <input value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://script.google.com/macros/s/..."
              style={{width:"100%",background:"#f5f9ff",border:"2px solid #e3f2fd",borderRadius:12,padding:"12px 14px",fontSize:12,outline:"none",fontFamily:"'Plus Jakarta Sans',sans-serif",color:"#1a237e",boxSizing:"border-box"}}/>
            <p style={{fontSize:11,color:"#90a4ae",marginTop:6,marginBottom:0}}>אם ריק — יחובר לגיליון ברירת המחדל</p>
          </div>
    ...
    <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:100}}>
      ...
      <div style={{padding:"20px 16px 0"}}>
        ...
        <Sec icon="📊" title="מדידות">
          <SliderField label="כלור" min={0} max={8} value={chlorine} onChange={v=>sf("chlorine",v)} unit=" ppm" warnAbove={3} optimal={1.5}/>
          <SliderField label="pH"   min={5} max={9} value={ph}       onChange={v=>sf("ph",v)}       warnAbove={8} warnBelow={6} optimal={7.4}/>
          <SliderField label="מלח"  min={0} max={6} value={salt}     onChange={v=>sf("salt",v)}     unit=" g/L" optimal={3.5}/>
        </Sec>
    ...
    return (
      <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:30}}>
        ...
        <div style={{background:C.white,padding:"8px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:6,overflowX:"auto",position:"sticky",top:0,zIndex:50,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          {[["daily","📋 חלוקת עבודה"],["progress","📊 התקדמות"],["hours","⏱️ שעות"],["qr","📷 QR"],["users","👥 משתמשים"]].map(([t,lbl])=>(
