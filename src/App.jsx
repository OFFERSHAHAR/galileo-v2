...
function pushEnabledKey(username) {
  return `galileo_push_enabled_${String(username || "").trim().toLowerCase()}`;
}
function getRememberedPushEnabled(username) {
  if (!username) return false;
  return localStorage.getItem(pushEnabledKey(username)) === "1";
}
function rememberPushEnabled(username, enabled) {
  if (!username) return;
  if (enabled) localStorage.setItem(pushEnabledKey(username), "1");
  else localStorage.removeItem(pushEnabledKey(username));
}
...
const APP_VERSION = "v2.6 ֲ· 08.05.2026";
    ...
    <div style={{position:"fixed",inset:0,zIndex:200,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
      ...
      <div style={{position:"relative",background:"#fff",borderRadius:"24px 24px 0 0",transform:vis?"translateY(0)":"translateY(100%)",transition:"transform 0.4s cubic-bezier(0.34,1.2,0.64,1)",maxHeight:"85vh",overflowY:"auto"}}>
        <div style={{padding:"16px 20px 8px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid #f0f4f8",position:"sticky",top:0,background:"#fff",zIndex:1}}>
          ...
          <Press onClick={close} style={{width:32,height:32,borderRadius:"50%",background:"#f0f4f8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,color:"#546e7a"}}>ג•</Press>
        </div>
...
function SliderField({label,min,max,step=0.1,value,onChange,optimal,unit="",warnAbove,warnBelow,large=false}) {
  ...
  let col=C.green,txt="׳×׳§׳™׳";
  if(warnAbove&&value>warnAbove){col=C.red;txt="ג ן¸ ׳’׳‘׳•׳”";}
  else if(warnBelow&&value<warnBelow){col=C.orange;txt="ג ן¸ ׳ ׳׳•׳";}
  else if(optimal&&Math.abs(value-optimal)<0.3){col=C.blue;txt="ג“ ׳׳•׳₪׳˜׳™׳׳׳™";}
  ...
  return (
    <div style={{marginBottom:6}}>
      ...
      <div dir="ltr" style={{display:"flex",justifyContent:"space-between",fontSize:large?12:10,color:C.muted}}>
        <span>{min}</span>{optimal&&<span style={{color:C.blue}}>׳׳•׳₪׳˜׳™׳׳׳™ {optimal}</span>}<span>{max}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{...card({marginBottom:8})}}>
      <Press onClick={()=>sf(expandKey,!isOpen)}
        ...
        <span style={{fontSize:16,color:C.blue,display:"inline-block",transform:isOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>ג–¾</span>
      </Press>
      ...
      <div style={{display:"flex",gap:6}}>
        {["׳×׳§׳™׳","׳׳ ׳×׳§׳™׳"].map(o=>(
          <Press key={o} onClick={()=>onChange(o)}
            style={{padding:"7px 14px",borderRadius:99,fontSize:12,fontWeight:800,background:value===o?(o==="׳×׳§׳™׳"?C.blue:C.red):"#f0f4f8",color:value===o?"#fff":C.muted,boxShadow:value===o?`0 4px 12px ${o==="׳×׳§׳™׳"?"rgba(21,101,192,0.3)":"rgba(198,40,40,0.3)"}`:  "none",transition:"all 0.2s"}}>
            {o}
          </Press>
        ))}
      </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      <div style={{position:"absolute",top:16,right:16,zIndex:10}}>
        <Press onClick={onClose} style={{width:40,height:40,borderRadius:"50%",background:"rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:18}}>ג•</Press>
      </div>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,fontWeight:600,marginBottom:16,textAlign:"center",padding:"0 20px"}}>׳›׳•׳•׳ ׳׳× ׳”׳׳¦׳׳׳” ׳-QR Code ׳©׳ ׳”׳׳§׳•׳—</p>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
        <p style={{color:"rgba(255,255,255,0.5)",fontSize:12}}>׳׳• ׳‘׳—׳¨ ׳×׳׳•׳ ׳” ׳©׳ QR:</p>
...
function LicenseScreen({ onDone, onSuperAdmin }) {
  ...
  const validate = async () => {
    ...
    if(res?.valid){
      ...
      setErr(res?.reason||"׳׳₪׳×׳— ׳׳ ׳×׳§׳™׳"); setLoading(false);
    }
  };
  return (
    <div dir="rtl" style={{minHeight:"100vh",background:`linear-gradient(145deg,#0d47a1,#1565c0,#1976d2)`,fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{width:"100%",maxWidth:380}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{fontSize:64,marginBottom:12,filter:"drop-shadow(0 0 24px rgba(255,255,255,0.3))"}}>נ</div>
          <h1 style={{color:"#fff",fontSize:26,fontWeight:900,margin:"0 0 6px",letterSpacing:"-0.5px"}}>POOLMANG.BY.OR2026</h1>
          <p style={{color:"rgba(255,255,255,0.55)",fontSize:14,margin:0}}>׳׳¢׳¨׳›׳× ׳ ׳™׳”׳•׳ ׳‘׳¨׳™׳›׳•׳× ׳׳§׳¦׳•׳¢׳™׳×</p>
        </div>
        <div style={{background:"rgba(255,255,255,0.1)",backdropFilter:"blur(20px)",borderRadius:24,padding:28,border:"1px solid rgba(255,255,255,0.2)",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            <h2 style={{color:"#fff",fontSize:18,fontWeight:900,margin:"0 0 6px"}}>׳”׳–׳ ׳׳₪׳×׳— ׳¨׳™׳©׳™׳•׳</h2>
            <p style={{color:"rgba(255,255,255,0.55)",fontSize:13,margin:0}}>׳§׳™׳‘׳׳× ׳׳₪׳×׳—? ׳”׳–׳ ׳׳•׳×׳• ׳›׳׳</p>
          </div>
      ...
      <div onClick={onSuperAdmin} style={{position:"fixed",bottom:16,left:16,fontSize:28,opacity:0.22,padding:10,zIndex:10,WebkitTapHighlightColor:"transparent",cursor:"pointer"}}>ג™ן¸</div>
    </div>
  );
}
...
const blank = () => ({
  ...
  elModel:"",elSerial:"",elDate:"",waterLevel:"׳×׳§׳™׳",clarity:"׳×׳§׳™׳",fat:"׳×׳§׳™׳",flow:"׳×׳§׳™׳",
  acid:false,phUpSupply:false,saltPkg:false,saltBags:1,poolStatus:"׳׳׳•׳–׳ ׳×",customStatusText:"",restrictedUntil:"",
...
function LicensesTab({C2, inp2, showMsg}) {
  ...
  const createLicense = async () => {
    if(!newLic.company||!newLic.sheetId){showMsg("ג ן¸ ׳׳׳ ׳©׳ ׳—׳‘׳¨׳” ׳•-Sheet ID");return;}
    const key = generateLicenseKey();
    await mgmtCall("saveLicense",{license:[key, newLic.company, newLic.sheetId, newLic.plan, "׳₪׳¢׳™׳", newLic.expiry||"", newLic.adminEmail||""]});
    setGenerated(key); setNewLic({company:"",sheetId:"",plan:"PRO",expiry:"",adminEmail:""}); loadLicenses();
  };
  const updateLicenseStatus = async (rowIndex, status) => { await mgmtCall("updateLicenseStatus",{rowIndex, status}); loadLicenses(); showMsg(`ג… ׳¡׳˜׳˜׳•׳¡ ׳¢׳•׳“׳›׳ ׳${status}`); };

  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{fontSize:12,fontWeight:800,color:C2.muted,letterSpacing:"0.1em",textTransform:"uppercase"}}>{licenses.length} ׳¨׳™׳©׳™׳•׳ ׳•׳×</div>
      ...
      {showForm&&(
        <div style={{background:C2.white,borderRadius:16,padding:16,marginBottom:16,border:`1px solid ${C2.border}`}}>
          <div style={{fontWeight:800,fontSize:14,color:C2.text,marginBottom:14}}>׳¨׳™׳©׳™׳•׳ ׳—׳“׳©</div>
          {[["company","׳©׳ ׳—׳‘׳¨׳” *"],["sheetId","Google Sheet ID *"],["adminEmail","׳׳™׳™׳ ׳׳“׳׳™׳"]].map(([k,lbl])=>(
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
            <div>
              <label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:4}}>׳×׳•׳›׳ ׳™׳×</label>
          ...
          <Press onClick={createLicense} style={{padding:"12px",borderRadius:12,background:`linear-gradient(135deg,${C2.blue},#42a5f5)`,color:"#fff",fontWeight:800,fontSize:14,textAlign:"center",marginBottom:generated?12:0}}>נ”‘ ׳¦׳•׳¨ ׳׳₪׳×׳— ׳¨׳™׳©׳™׳•׳</Press>
          {generated&&(
            <div style={{background:"#e8f5e9",borderRadius:12,padding:16,border:"1px solid #c8e6c9",textAlign:"center",marginTop:12}}>
              <div style={{fontSize:11,fontWeight:700,color:C2.green,marginBottom:8}}>ג… ׳©׳׳— ׳׳׳§׳•׳—:</div>
              ...
              <Press onClick={()=>{ navigator.clipboard?.writeText(generated); showMsg("נ“‹ ׳”׳•׳¢׳×׳§!"); }} style={{background:C2.green,color:"#fff",borderRadius:99,padding:"8px 20px",fontSize:13,fontWeight:700,display:"inline-block"}}>נ“‹ ׳”׳¢׳×׳§ ׳׳₪׳×׳—</Press>
            </div>
          )}
        </div>
      )}
      {loading&&<div style={{textAlign:"center",padding:32,color:C2.muted}}>ג³ ׳˜׳•׳¢׳...</div>}
      {!loading&&licenses.length===0&&<div style={{background:C2.white,borderRadius:16,padding:32,textAlign:"center",color:C2.muted}}><div style={{fontSize:32,marginBottom:8}}>נ”‘</div><div style={{fontWeight:700}}>׳׳™׳ ׳¨׳™׳©׳™׳•׳ ׳•׳× ׳¢׳“׳™׳™׳</div></div>}
      {licenses.map((lic,i)=>{
        const [key,company,sheetId,plan,status,expiry] = lic;
        return (
          <div key={i} style={{background:C2.white,borderRadius:16,padding:16,marginBottom:10,border:`1px solid ${status==="׳׳•׳©׳”׳”"?C2.red+"33":C2.border}`,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
            ...
            <div style={{fontSize:11,color:C2.muted,marginBottom:10}}>נ“… ׳×׳•׳§׳£: {expiry||"ג€”"}</div>
            <div style={{display:"flex",gap:6}}>
              {["׳₪׳¢׳™׳","׳׳•׳©׳”׳”"].map(s=>(
                <Press key={s} onClick={()=>updateLicenseStatus(i+2,s)} style={{padding:"6px 14px",borderRadius:99,fontSize:12,fontWeight:800,background:(status||"׳₪׳¢׳™׳")===s?(s==="׳₪׳¢׳™׳"?"#e8f5e9":"#ffebee"):"#f0f4f8",color:(status||"׳₪׳¢׳™׳")===s?(s==="׳₪׳¢׳™׳"?C2.green:C2.red):C2.muted}}>
                  {s==="׳₪׳¢׳™׳"?"ג… ׳₪׳¢׳™׳":"ג›” ׳׳•׳©׳”׳”"}
                </Press>
              ))}
              <Press onClick={()=>{ navigator.clipboard?.writeText(key); showMsg("נ“‹ ׳׳₪׳×׳— ׳”׳•׳¢׳×׳§!"); }} style={{padding:"6px 14px",borderRadius:99,fontSize:12,fontWeight:800,background:"#e3f2fd",color:C2.blue}}>נ“‹ ׳”׳¢׳×׳§</Press>
            </div>
          </div>
        );
      })}
...
function SuperAdminScreen({ onClose }) {
  ...
  const [newClient, setNewClient] = useState({name:"",contact:"",phone:"",email:"",plan:"PRO",status:"׳₪׳¢׳™׳",sheetId:"",notes:""});
  ...
  const login = () => { if(pass===getSuperPass()){ setAuth(true); loadData(); haptic("success"); } else { setErr("׳¡׳™׳¡׳׳” ׳©׳’׳•׳™׳”"); haptic("medium"); } };
  ...
  const saveClient = async (row) => { setSaving(true); await mgmtCall("saveMgmtClient", { row }); await loadData(); setSaving(false); showMsg("ג… ׳ ׳©׳׳¨"); haptic("success"); };
  const deleteClient = async (rowIndex) => { if(!window.confirm("׳׳׳—׳•׳§ ׳׳§׳•׳— ׳–׳”?")) return; setSaving(true); await mgmtCall("deleteMgmtClient", { rowIndex }); await loadData(); setSaving(false); showMsg("נ—‘ן¸ ׳׳§׳•׳— ׳ ׳׳—׳§"); };
  const updateClientStatus = async (rowIndex, status) => { setSaving(true); await mgmtCall("updateMgmtClientStatus", { rowIndex, status }); await loadData(); setSaving(false); showMsg("ג… ׳¢׳•׳“׳›׳"); };
  const updateIssueStatus = async (idx, newStatus) => { const updated = [...issues]; updated[idx] = [...updated[idx]]; updated[idx][5] = newStatus; setIssues(updated); await mgmtCall("updateMgmtIssueStatus", { rowIndex: idx+2, status: newStatus }); showMsg("ג… ׳¡׳˜׳˜׳•׳¡ ׳¢׳•׳“׳›׳"); };
  const addIssueNote = async (idx, note) => { if(!note.trim()) return; const updated = [...issues]; updated[idx] = [...updated[idx]]; updated[idx][6] = note; setIssues(updated); await mgmtCall("updateMgmtIssueStatus", { rowIndex: idx+2, status: updated[idx][5], note }); setIssueNote({}); showMsg("ג… ׳”׳¢׳¨׳” ׳ ׳•׳¡׳₪׳”"); };
  const filteredIssues = issues.filter(i => !dateFilter || String(i[2]).slice(0,10)===dateFilter);
  const pendingCount = issues.filter(i=>i[5]==="׳₪׳×׳•׳—"||!i[5]).length;
  ...
  const statusColor = s => s==="׳˜׳•׳₪׳"?"#e8f5e9":s==="׳‘׳˜׳™׳₪׳•׳"?"#e3f2fd":s==="׳”׳•׳¢׳‘׳¨"?"#f3e5f5":"#fff8e1";
  const statusTextColor = s => s==="׳˜׳•׳₪׳"?C2.green:s==="׳‘׳˜׳™׳₪׳•׳"?C2.blue:s==="׳”׳•׳¢׳‘׳¨"?"#6a1b9a":C2.orange;

  const ClientForm = ({data, onSave, onCancel}) => {
    const [f, setF] = useState(data);
    return (
      <div style={{background:C2.white,borderRadius:16,padding:16,marginBottom:16,border:`1px solid ${C2.border}`}}>
        {[["name","׳©׳ ׳—׳‘׳¨׳” *"],["contact","׳׳™׳© ׳§׳©׳¨"],["phone","׳˜׳׳₪׳•׳"],["email","׳׳™׳™׳"],["sheetId","Sheet ID"],["logoUrl","URL ׳׳•׳’׳• (PNG ׳©׳§׳•׳£ ג€” ׳׳•׳¦׳’ ׳‘׳©׳׳™׳¨׳” ׳׳׳¡׳ ׳”׳‘׳™׳×)"]].map(([k,lbl])=>(
          <div key={k} style={{marginBottom:10}}>
            <label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:4}}>{lbl}</label>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          <div><label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:4}}>׳׳ ׳•׳™</label><select value={f.plan||"PRO"} onChange={e=>setF(x=>({...x,plan:e.target.value}))} style={{...inp2}}><option>PRO</option><option>Basic</option><option>׳ ׳™׳¡׳™׳•׳</option></select></div>
          <div><label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:4}}>׳¡׳˜׳˜׳•׳¡</label><select value={f.status||"׳₪׳¢׳™׳"} onChange={e=>setF(x=>({...x,status:e.target.value}))} style={{...inp2}}><option>׳₪׳¢׳™׳</option><option>׳׳•׳©׳”׳”</option><option>׳ ׳™׳¡׳™׳•׳</option></select></div>
        </div>
        <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:4}}>׳”׳¢׳¨׳•׳×</label><textarea value={f.notes||""} onChange={e=>setF(x=>({...x,notes:e.target.value}))} rows={2} style={{...inp2,resize:"none"}}/></div>
        <div style={{display:"flex",gap:8}}>
          <Press onClick={()=>onSave(f)} style={{flex:1,padding:"12px",borderRadius:12,background:`linear-gradient(135deg,${C2.blue},#42a5f5)`,color:"#fff",fontWeight:800,fontSize:13,textAlign:"center"}}>{saving?"ג³":"נ’¾ ׳©׳׳•׳¨"}</Press>
          <Press onClick={onCancel} style={{padding:"12px 16px",borderRadius:12,background:"#f0f4f8",color:C2.muted,fontWeight:700,fontSize:13}}>׳‘׳™׳˜׳•׳</Press>
        </div>
      </div>
    );
  };
        ...
        <div style={{background:`linear-gradient(145deg,#0d47a1,#1565c0,#1976d2)`,padding:"28px 20px 20px",position:"relative",overflow:"hidden",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative"}}>
            <div>
              ...
              {auth&&<div style={{color:"rgba(255,255,255,0.6)",fontSize:12,marginTop:2}}>{clients.length} ׳׳§׳•׳—׳•׳× ֲ· {pendingCount} ׳×׳§׳׳•׳× ׳׳׳×׳™׳ ׳•׳×</div>}
            </div>
            <div style={{display:"flex",gap:8}}>
              {auth&&<Press onClick={()=>{loadData();haptic();}} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 12px",color:"#fff",fontSize:13,fontWeight:700}}>נ”„</Press>}
              <Press onClick={close} style={{width:36,height:36,borderRadius:"50%",background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:16}}>ג•</Press>
            </div>
          </div>
        </div>
        ...
        {!auth?(
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
            <div style={{background:"#fff",borderRadius:24,padding:28,width:"100%",maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
              <div style={{fontSize:48,textAlign:"center",marginBottom:12}}>נ”</div>
              <div style={{fontWeight:900,fontSize:18,color:C2.text,textAlign:"center",marginBottom:20}}>׳›׳ ׳™׳¡׳” ׳׳׳•׳‘׳˜׳—׳×</div>
              <input type="password" value={pass} onChange={e=>{setPass(e.target.value);setErr("");}} placeholder="׳¡׳™׳¡׳׳” ׳¡׳•׳“׳™׳×" style={{...inp2,marginBottom:err?8:16}} onKeyDown={e=>e.key==="Enter"&&login()}/>
              {err&&<div style={{background:"#ffebee",borderRadius:10,padding:"8px 14px",marginBottom:12,color:C2.red,fontSize:13,fontWeight:700,textAlign:"center"}}>ג ן¸ {err}</div>}
              <Press onClick={login} style={{padding:"14px",borderRadius:14,background:`linear-gradient(135deg,${C2.blue},#42a5f5)`,color:"#fff",fontWeight:900,fontSize:15,textAlign:"center",boxShadow:"0 6px 20px rgba(21,101,192,0.4)"}}>׳›׳ ׳™׳¡׳” ג†’</Press>
            </div>
          </div>
        ):(
          <>
            <div style={{background:C2.white,padding:"8px 12px",borderBottom:`1px solid ${C2.border}`,display:"flex",gap:6,flexShrink:0,overflowX:"auto",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
              {[["issues",`נ”§ ׳×׳§׳׳•׳×${pendingCount>0?` (${pendingCount})`:""}`],["clients","נ‘¥ ׳׳§׳•׳—׳•׳×"],["licenses","נ”‘ ׳¨׳™׳©׳™׳•׳ ׳•׳×"],["stats","נ“ ׳¡׳˜׳˜׳™׳¡׳˜׳™׳§׳•׳×"],["settings","ג™ן¸ ׳”׳’׳“׳¨׳•׳×"]].map(([t,lbl])=>(
            ...
            <div style={{flex:1,overflowY:"auto",padding:"16px"}}>
              {loading&&<div style={{textAlign:"center",padding:60,color:C2.muted}}><div style={{fontSize:32,marginBottom:12}}>ג³</div><div style={{fontSize:14,fontWeight:700}}>׳˜׳•׳¢׳...</div></div>}
              {tab==="issues"&&!loading&&(
                <div>
                  {pendingCount>0&&<div style={{background:"#fff8e1",border:"1px solid #ffe082",borderRadius:16,padding:"14px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:22}}>נ””</span><div><div style={{fontWeight:800,fontSize:14,color:C2.orange}}>{pendingCount} ׳×׳§׳׳•׳× ׳׳׳×׳™׳ ׳•׳× ׳׳˜׳™׳₪׳•׳</div><div style={{fontSize:11,color:"#bf6900",marginTop:2}}>׳׳—׳¥ ׳¢׳ ׳¡׳˜׳˜׳•׳¡ ׳׳¢׳“׳›׳•׳</div></div></div>}
                  <div style={{display:"flex",gap:8,marginBottom:16}}>
                    <input type="date" value={dateFilter} onChange={e=>setDateFilter(e.target.value)} style={{...inp2,flex:1,fontSize:12}}/>
                    {dateFilter&&<Press onClick={()=>setDateFilter("")} style={{padding:"10px 14px",borderRadius:10,background:"#ffebee",color:C2.red,fontWeight:700,fontSize:12}}>ג•</Press>}
                  </div>
                  {filteredIssues.length===0&&<div style={{background:C2.white,borderRadius:16,padding:32,textAlign:"center",color:C2.muted}}><div style={{fontSize:32,marginBottom:8}}>ג…</div><div style={{fontWeight:700}}>׳׳™׳ ׳×׳§׳׳•׳×</div></div>}
                  {filteredIssues.map((issue,i)=>{
                    const priority=issue[4]||"׳¨׳’׳™׳"; const status=issue[5]||"׳₪׳×׳•׳—";
                    const priColor=priority==="׳§׳¨׳™׳˜׳™"?C2.red:priority==="׳“׳—׳•׳£"?C2.orange:C2.blue;
                    ...
                    return (
                      <div key={i} style={{background:C2.white,borderRadius:16,padding:16,marginBottom:12,border:`2px solid ${priColor}22`,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
                        ...
                        <div style={{fontSize:11,color:C2.muted,marginBottom:10}}>נ“… {issue[2]}</div>
                        {issue[6]&&<div style={{background:"#e8f5e9",borderRadius:10,padding:"8px 12px",fontSize:12,color:C2.green,fontWeight:700,marginBottom:10}}>נ“ {issue[6]}</div>}
                        ...
                        <div style={{display:"flex",gap:8}}>
                          {!showNote?(
                            <Press onClick={()=>setIssueNote({...issueNote,[realIdx]:""})} style={{padding:"7px 14px",borderRadius:10,background:"#f0f4f8",color:C2.muted,fontSize:12,fontWeight:700}}>ג• ׳”׳•׳¡׳£ ׳”׳¢׳¨׳”</Press>
                          ):(
                            <>
                              <input value={issueNote[realIdx]||""} onChange={e=>setIssueNote({...issueNote,[realIdx]:e.target.value})} placeholder="׳›׳×׳•׳‘ ׳”׳¢׳¨׳”..." style={{...inp2,flex:1,padding:"8px 12px",fontSize:12}}/>
                              <Press onClick={()=>addIssueNote(realIdx,issueNote[realIdx]||"")} style={{padding:"8px 14px",borderRadius:10,background:C2.blue,color:"#fff",fontWeight:700,fontSize:12}}>׳©׳׳•׳¨</Press>
                              <Press onClick={()=>setIssueNote({})} style={{padding:"8px 10px",borderRadius:10,background:"#ffebee",color:C2.red,fontWeight:700,fontSize:12}}>ג•</Press>
                            </>
                          )}
                        </div>
                      </div>
              ...
              {tab==="clients"&&!loading&&(
                <div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <div style={{fontSize:12,fontWeight:800,color:C2.muted,letterSpacing:"0.1em",textTransform:"uppercase"}}>{clients.length} ׳׳§׳•׳—׳•׳×</div>
                    <Press onClick={()=>setShowAddClient(!showAddClient)} style={{padding:"8px 16px",borderRadius:99,background:showAddClient?"#ffebee":`linear-gradient(135deg,${C2.blue},#42a5f5)`,color:showAddClient?C2.red:"#fff",fontWeight:800,fontSize:12,boxShadow:showAddClient?"none":"0 4px 12px rgba(21,101,192,0.3)"}}>{showAddClient?"ג• ׳‘׳™׳˜׳•׳":"ג• ׳׳§׳•׳— ׳—׳“׳©"}</Press>
                  </div>
                  {showAddClient&&<ClientForm data={newClient} onCancel={()=>setShowAddClient(false)} onSave={async(f)=>{ if(!f.name?.trim()){showMsg("ג ן¸ ׳ ׳ ׳׳”׳–׳™׳ ׳©׳ ׳—׳‘׳¨׳”");return;} await saveClient([Date.now(),f.name,f.contact,f.phone,f.email,f.plan,f.status,f.sheetId,"","","","","","",f.notes,f.logoUrl||""]); setNewClient({name:"",contact:"",phone:"",email:"",plan:"PRO",status:"׳₪׳¢׳™׳",sheetId:"",notes:"",logoUrl:""}); setShowAddClient(false); }}/>}
                  {clients.length===0&&!showAddClient&&<div style={{background:C2.white,borderRadius:16,padding:32,textAlign:"center",color:C2.muted}}><div style={{fontSize:32,marginBottom:8}}>נ‘¥</div><div style={{fontWeight:700}}>׳׳™׳ ׳׳§׳•׳—׳•׳× ׳¢׳“׳™׳™׳</div></div>}
                  {clients.map((c,i)=>(
                    <div key={i}>
                      {editClient===i?(
                        ...
                        <div style={{background:C2.white,borderRadius:16,padding:16,marginBottom:10,boxShadow:"0 2px 12px rgba(0,0,0,0.06)",border:`1px solid ${c[6]==="׳׳•׳©׳”׳”"?C2.red+"33":C2.border}`}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                            <div><div style={{fontWeight:900,fontSize:15,color:C2.text}}>{c[1]}</div><div style={{fontSize:12,color:C2.muted,marginTop:3}}>{c[2]} ֲ· {c[3]}</div></div>
                            <div style={{display:"flex",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
                              ...
                              <span style={{background:c[6]==="׳₪׳¢׳™׳"?"#e8f5e9":c[6]==="׳׳•׳©׳”׳”"?"#ffebee":"#fff8e1",color:c[6]==="׳₪׳¢׳™׳"?C2.green:c[6]==="׳׳•׳©׳”׳”"?C2.red:C2.orange,borderRadius:99,padding:"4px 10px",fontSize:11,fontWeight:800}}>{c[6]||"׳₪׳¢׳™׳"}</span>
                            </div>
                          </div>
                          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
                            {c[7]&&<a href={`https://docs.google.com/spreadsheets/d/${c[7]}`} target="_blank" rel="noreferrer" style={{background:"#e3f2fd",color:C2.blue,borderRadius:99,padding:"4px 12px",fontSize:11,fontWeight:700,textDecoration:"none"}}>נ“ ׳’׳™׳׳™׳•׳</a>}
                            {c[4]&&<a href={`mailto:${c[4]}`} style={{background:"#f5f5f5",color:"#555",borderRadius:99,padding:"4px 12px",fontSize:11,fontWeight:700,textDecoration:"none"}}>ג‰ן¸ ׳׳™׳™׳</a>}
                            {c[3]&&<a href={`tel:${c[3]}`} style={{background:"#f5f5f5",color:"#555",borderRadius:99,padding:"4px 12px",fontSize:11,fontWeight:700,textDecoration:"none"}}>נ“</a>}
                          </div>
                          <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
                            {["׳₪׳¢׳™׳","׳׳•׳©׳”׳”","׳ ׳™׳¡׳™׳•׳"].map(s=>(<Press key={s} onClick={()=>updateClientStatus(i+2,s)} style={{padding:"5px 12px",borderRadius:99,fontSize:11,fontWeight:800,background:(c[6]||"׳₪׳¢׳™׳")===s?(s==="׳₪׳¢׳™׳"?"#e8f5e9":s==="׳׳•׳©׳”׳”"?"#ffebee":"#fff8e1"):"#f0f4f8",color:(c[6]||"׳₪׳¢׳™׳")===s?(s==="׳₪׳¢׳™׳"?C2.green:s==="׳׳•׳©׳”׳”"?C2.red:C2.orange):C2.muted}}>{s}</Press>))}
                          </div>
                          <div style={{display:"flex",gap:8,marginTop:10}}>
                            <Press onClick={()=>setEditClient(i)} style={{flex:1,padding:"8px",borderRadius:10,background:"#e3f2fd",color:C2.blue,fontWeight:700,fontSize:12,textAlign:"center"}}>גן¸ ׳¢׳¨׳™׳›׳”</Press>
                            <Press onClick={()=>deleteClient(i+2)} style={{padding:"8px 14px",borderRadius:10,background:"#ffebee",color:C2.red,fontWeight:700,fontSize:12}}>נ—‘ן¸</Press>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              {tab==="stats"&&!loading&&(
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                    {[["נ‘¥","׳¡׳”\"׳› ׳׳§׳•׳—׳•׳×",clients.length,C2.blue],["ג…","׳׳§׳•׳—׳•׳× ׳₪׳¢׳™׳׳™׳",clients.filter(c=>c[6]==="׳₪׳¢׳™׳"||!c[6]).length,C2.green],["נ”§","׳×׳§׳׳•׳× ׳₪׳×׳•׳—׳•׳×",issues.filter(i=>i[5]==="׳₪׳×׳•׳—"||!i[5]).length,C2.orange],["נ’","׳׳ ׳•׳™ PRO",clients.filter(c=>c[5]==="PRO").length,C2.blue]].map(([ic,lbl,val,col])=>(
              ...
              {tab==="settings"&&(
                <div style={{background:C2.white,borderRadius:16,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,0.06)",border:`1px solid ${C2.border}`}}>
                  <div style={{fontWeight:900,fontSize:16,color:C2.text,marginBottom:20}}>נ”‘ ׳©׳™׳ ׳•׳™ ׳¡׳™׳¡׳׳× ׳›׳ ׳™׳¡׳”</div>
                  <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:6}}>׳¡׳™׳¡׳׳” ׳—׳“׳©׳”</label><input type="password" value={newPass} onChange={e=>setNewPass(e.target.value)} style={inp2} placeholder="׳׳₪׳—׳•׳× 6 ׳×׳•׳•׳™׳"/></div>
                  <div style={{marginBottom:16}}><label style={{fontSize:11,fontWeight:700,color:C2.muted,display:"block",marginBottom:6}}>׳׳™׳׳•׳× ׳¡׳™׳¡׳׳”</label><input type="password" value={newPass2} onChange={e=>setNewPass2(e.target.value)} style={inp2} placeholder="׳”׳–׳ ׳©׳•׳‘"/></div>
                  {passMsg&&<div style={{background:passMsg.includes("ג…")?"#e8f5e9":"#ffebee",borderRadius:10,padding:"10px 14px",marginBottom:14,color:passMsg.includes("ג…")?C2.green:C2.red,fontSize:13,fontWeight:700,textAlign:"center"}}>{passMsg}</div>}
                  <Press onClick={()=>{ if(!newPass||newPass.length<6){setPassMsg("ג ן¸ ׳¡׳™׳¡׳׳” ׳—׳™׳™׳‘׳× ׳׳”׳™׳•׳× ׳׳₪׳—׳•׳× 6 ׳×׳•׳•׳™׳");return;} if(newPass!==newPass2){setPassMsg("ג ן¸ ׳”׳¡׳™׳¡׳׳׳•׳× ׳׳ ׳×׳•׳׳׳•׳×");return;} setSuperPass(newPass); setNewPass(""); setNewPass2(""); setPassMsg("ג… ׳¡׳™׳¡׳׳” ׳¢׳•׳“׳›׳ ׳”!"); haptic("success"); setTimeout(()=>setPassMsg(""),3000); }} style={{padding:"14px",borderRadius:14,background:`linear-gradient(135deg,${C2.blue},#42a5f5)`,color:"#fff",fontWeight:900,fontSize:15,textAlign:"center",boxShadow:"0 6px 20px rgba(21,101,192,0.35)"}}>׳¢׳“׳›׳ ׳¡׳™׳¡׳׳”</Press>
                </div>
              )}
            </div>
          </>
...
const [screen,setScreen] = useState(() => {
  ...
  const [pushEnabled, setPushEnabled] = useState(()=>getRememberedPushEnabled(user?.username));
  const [pushCardOpen, setPushCardOpen] = useState(()=>!getRememberedPushEnabled(user?.username));
  ...
  const [opIssuePriority,setOpIssuePriority] = useState("׳¨׳’׳™׳");
  ...
  useEffect(() => {
    const enabled = getRememberedPushEnabled(user?.username);
    setPushEnabled(enabled);
    setAction("push", enabled ? "success" : "idle");
    setPushCardOpen(!enabled);
  }, [user?.username]);
  ...
  const reportOperatorOptions = [...new Set([...opNames, ...sheetReports.map(r=>r.operator).filter(Boolean), ...reports.map(r=>r.operator).filter(Boolean)])];
  ...
  const normalizeDay = (d) => String(d||"").trim().replace(/^׳$/,"׳¨׳׳©׳•׳").replace(/^׳‘$/,"׳©׳ ׳™").replace(/^׳’$/,"׳©׳׳™׳©׳™").replace(/^׳“$/,"׳¨׳‘׳™׳¢׳™").replace(/^׳”$/,"׳—׳׳™׳©׳™").replace(/^׳•$/,"׳©׳™׳©׳™").replace(/^׳©$/,"׳©׳‘׳×").replace(/^1$/,"׳¨׳׳©׳•׳").replace(/^2$/,"׳©׳ ׳™").replace(/^3$/,"׳©׳׳™׳©׳™").replace(/^4$/,"׳¨׳‘׳™׳¢׳™").replace(/^5$/,"׳—׳׳™׳©׳™").replace(/^6$/,"׳©׳™׳©׳™").replace(/^7$/,"׳©׳‘׳×");
  ...
  const enablePushForCurrentUser = async () => {
    ...
    if (ok === true) {
      rememberPushEnabled(user.username, true);
      setPushEnabled(true);
  ...
  const PushSetupCard = ({compact=false}) => (
    <div style={{...card({marginBottom: compact ? 10 : 12,background: pushEnabled ? "#e8f5e9" : "#e3f2fd",border: `1px solid ${pushEnabled ? C.green : C.lightBlue}`}),padding: compact ? "10px 14px" : "12px 16px"}}>
      <Press onClick={()=>setPushCardOpen(o=>!o)} style={{display:"flex",alignItems:"center",gap:10}}>
        <span style={{fontSize:18}}>נ””</span>
        <div style={{flex:1}}>
          <div style={{fontWeight:800,fontSize:13,color:pushEnabled ? C.green : C.blue}}>׳”׳×׳¨׳׳•׳× ׳׳™׳©׳™׳•׳×</div>
          {pushCardOpen&&<div style={{fontSize:11,color:C.muted}}>{pushEnabled ? "התראות פעילות למשתמש הזה" : "נדרש לקבלת משימות ועדכונים לפי משתמש"}</div>}
        </div>
        <span style={{fontSize:12,fontWeight:800,color:pushEnabled ? C.green : C.blue,display:"inline-flex",alignItems:"center",gap:8}}>
          {pushEnabled ? "פעיל" : actionLabel("push",{idle:"הפעל",loading:"...",success:"✓",error:"נסה שוב"})}
          <span style={{fontSize:14,display:"inline-block",transform:pushCardOpen?"rotate(180deg)":"none",transition:"transform 0.2s"}}>ג–¾</span>
        </span>
      </Press>
      {pushCardOpen&&(
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>
          <Press onClick={enablePushForCurrentUser} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:99,background:pushEnabled ? C.green : C.blue,color:"#fff",fontSize:12,fontWeight:800}}>
            <span>נ””</span>
            <span>{pushEnabled ? "פעיל" : actionLabel("push",{idle:"הפעל",loading:"מפעיל...",success:"הופעל",error:"נסה שוב"})}</span>
          </Press>
          <Press onClick={resetPushForCurrentUser} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:99,background:"#fff",border:`1px solid ${C.border}`,color:C.muted,fontSize:12,fontWeight:800}}>
            <span>ג†»</span>
            <span>{actionLabel("pushReset",{idle:"׳׳™׳₪׳•׳¡ ׳”׳×׳¨׳׳•׳×",loading:"׳׳׳₪׳¡...",success:"׳׳•׳₪׳¡",error:"׳ ׳¡׳” ׳©׳•׳‘"})}</span>
          </Press>
        </div>
      )}
    </div>
  ...
  const InstallAppCard = ({compact=false}) => {
    ...
    return (
      ...
      >
        <span style={{fontSize:18}}>ג¬‡ן¸</span>
        <div style={{flex:1}}>
          <div style={{fontWeight:800,fontSize:13,color:C.green}}>׳”׳×׳§׳ ׳׳₪׳׳™׳§׳¦׳™׳”</div>
          <div style={{fontSize:11,color:C.muted}}>׳₪׳×׳— ׳‘׳׳™ ׳“׳₪׳“׳₪׳ ׳•׳§׳‘׳ ׳—׳•׳•׳™׳” ׳™׳¦׳™׳‘׳” ׳™׳•׳×׳¨</div>
        </div>
        <span style={{fontSize:12,fontWeight:800,color:C.green}}>
          {actionLabel("install",{idle:"׳”׳×׳§׳",loading:"ג³",success:"ג…",manual:"ג‹®",error:"׳ ׳¡׳” ׳©׳•׳‘"})}
        </span>
      </Press>
    );
  };
  ...
  const saveTask = async (task) => {
    ...
    const logEntry={at:nowStr(),note:note||(isEdit?"׳׳©׳™׳׳” ׳¢׳•׳“׳›׳ ׳”":"נ“‹ ׳׳©׳™׳׳” ׳—׳“׳©׳” ׳”׳•׳§׳¦׳×׳” ׳׳"),by:user?.name,needsAck:true,ackedBy:[]};
    ...
    if(isEdit) await sendNotificationToOperators(cleanTask.operators, "נ“‹ ׳׳©׳™׳׳” ׳¢׳•׳“׳›׳ ׳”", `${cleanTask.client?.split(" - ")[0] || ""} ג€” ${fmtDate(cleanTask.date)}`);
    showToast(isEdit?"גן¸ ׳׳©׳™׳׳” ׳¢׳•׳“׳›׳ ׳”":"ג… ׳׳©׳™׳׳” ׳ ׳•׳¡׳₪׳”");
  };
  ...
  const updateTask = async (id,changes,logNote,isAdmin=false) => {
    ...
    if(isAdmin) {
      ...
      await sendNotificationToOperators(changedTask?.operators || [], "נ“‹ ׳׳©׳™׳׳” ׳¢׳•׳“׳›׳ ׳”", `${changedTask?.client?.split(" - ")[0] || ""} ג€” ${logNote}`);
    }
  };
  ...
  const ackChange = async (taskId,logIdx) => {
    ...
    setTasks(newTasks); if(sheetId) await sheetCall("saveTasks",{tasks:newTasks}); showToast("ג“ ׳§׳™׳‘׳׳× ׳׳™׳©׳•׳¨ ׳ ׳©׳׳—");
  };

  const removeOp=async(id,n)=>{const t=tasks.find(x=>x.id===id);if(!t)return;await updateTask(id,{operators:t.operators.filter(o=>o!==n)},`׳”׳•׳¡׳¨ ${n} ׳׳”׳׳©׳™׳׳”`,true);await sendNotificationToOperators([n], "נ“‹ ׳”׳•׳¡׳¨׳× ׳׳׳©׳™׳׳”", `${t.client?.split(" - ")[0] || ""} ג€” ${fmtDate(t.date)}`);};
  const addOp=(id,n)=>{const t=tasks.find(x=>x.id===id);if(!t||t.operators.includes(n))return;updateTask(id,{operators:[...t.operators,n]},`׳ ׳•׳¡׳£ ${n} ׳׳׳©׳™׳׳”`,true);};
  const markDone=(id)=>updateTask(id,{status:"done"},"׳“׳•׳— ׳”׳•׳’׳© ג€” ׳‘׳•׳¦׳¢׳”",false);

  const handleStartWork = () => { const now=new Date().toLocaleTimeString("he-IL",{hour:"2-digit",minute:"2-digit"}); localStorage.setItem("galileo_workstart",now); setWorkStart(now); haptic("medium"); showToast("ג–¶ ׳™׳•׳ ׳¢׳‘׳•׳“׳” ׳”׳×׳—׳™׳!"); };
  const handleEndWork = () => {
    if(!workStart)return;
    ...
    setWorkStart(null); haptic("success"); showToast(`ג¹ ${totalStr} ׳©׳¢׳•׳× ׳¢׳‘׳•׳“׳” ׳ ׳©׳׳¨׳•`); if(sheetId) sheetCall("saveWorkLog",{log});
  };

  const buildWA = (r) => {
    const name=r.client?.split(" - ")[0]||"׳׳§׳•׳— ׳™׳§׳¨"; const company = getCompany().name || "POOLMANG";
    const statusLine=r.poolStatus==="׳׳—׳¨"?`ג ן¸ *׳ ׳“׳¨׳©׳× ׳×׳©׳•׳׳× ׳׳‘:*\n${r.customStatusText}${r.restrictedUntil?`\n׳”׳‘׳¨׳™׳›׳” ׳׳ ׳–׳׳™׳ ׳” ׳¢׳“ ${fmtDate(r.restrictedUntil)}`:""}` :"ג… ׳”׳‘׳¨׳™׳›׳” ׳׳׳•׳–׳ ׳× ׳•׳׳•׳›׳ ׳” ׳׳©׳™׳׳•׳© ׳׳׳";
    return `*׳˜׳™׳₪׳•׳ ׳‘׳¨׳™׳›׳” ׳”׳•׳©׳׳!*\n\n׳©׳׳•׳ ${name},\n\n${user?.name} ׳¡׳™׳™׳ ׳׳× ׳”׳˜׳™׳₪׳•׳ ׳‘׳‘׳¨׳™׳›׳” ׳©׳׳›׳ ׳”׳™׳•׳.\n\n${statusLine}${r.notes?`\n\nנ“ ${r.notes}`:""}\n\n׳×׳׳™׳“ ׳›׳׳ ׳‘׳©׳‘׳™׳׳›׳,\n_׳¦׳•׳•׳× ${company}_`;
  };
  ...
  const handleSubmit = async () => {
    ...
    const supplyLabel=[acid&&"׳—׳•׳׳¦׳× ׳׳׳—",phUpSupply&&"׳׳¢׳׳” pH",saltPkg&&`׳׳׳— ֳ—${saltBags}`].filter(Boolean).join(", ");
    if(client&&(acid||phUpSupply||saltPkg)){ const newDB={...supplyDB,[client]:{acid,phUpSupply,saltPkg,saltBags,updatedAt:fmtDate(reportDate)}}; setSupplyDB(newDB); if(sheetId){const rows=Object.entries(newDB).map(([c,v])=>[c,v.acid?"׳›׳":"׳׳",v.phUpSupply?"׳›׳":"׳׳",v.saltPkg?"׳›׳":"׳׳",v.saltBags||0,v.updatedAt]);await sheetCall("saveSupplyDB",{rows});} }
...
const report = {
  ...
  const SLIDER_CONFIGS = [
    {key:"chlorine",label:"׳›׳׳•׳¨",min:0,max:8,step:0.1,unit:" ppm",warnAbove:3,optimal:1.5,val:chlorine,fn:v=>sf("chlorine",v)},
    {key:"ph",label:"pH",min:5,max:9,step:0.1,unit:"",warnAbove:8,warnBelow:6,optimal:7.4,val:ph,fn:v=>sf("ph",v)},
    {key:"salt",label:"׳׳׳—",min:0,max:6,step:0.1,unit:" g/L",optimal:3.5,val:salt,fn:v=>sf("salt",v)},
    {key:"chlora",label:"׳˜׳‘׳׳™׳•׳× ׳›׳׳•׳¨ (TAB)",min:0,max:5,step:0.25,unit:"",val:form.chlora??0,fn:v=>sf("chlora",v)},
    {key:"hth",label:"HTH",min:0,max:5,step:0.5,unit:" cups",val:form.hth??0,fn:v=>sf("hth",v)},
    {key:"phUp",label:"׳׳¢׳׳” ׳—׳•׳׳¦׳™׳•׳× pH",min:0,max:5,step:0.5,unit:" ׳›׳•׳¡׳•׳×",val:form.phUp??0,fn:v=>sf("phUp",v)},
    {key:"acidLiters",label:"׳—׳•׳׳¦׳× ׳׳׳—",min:0,max:5,step:0.5,unit:" L",val:form.acidLiters??0,fn:v=>sf("acidLiters",v)},
  ];
    ...
    <div dir="rtl" style={{minHeight:"100vh",background:"linear-gradient(145deg,#0d47a1,#1565c0,#1976d2)",fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      ...
      <div style={{width:"100%",maxWidth:360}}>
        <div style={{textAlign:"center",marginBottom:36}} onPointerDown={()=>{ logoLongPress.current = setTimeout(()=>{ haptic("success"); setShowSetup(true); }, 3000); }} onPointerUp={()=>clearTimeout(logoLongPress.current)} onPointerLeave={()=>clearTimeout(logoLongPress.current)}>
          {(()=>{ const logoUrl = getCompany().logoUrl; return logoUrl ? (<img src={logoUrl} alt="logo" style={{width:80,height:80,objectFit:"contain",marginBottom:12,filter:"drop-shadow(0 0 20px rgba(255,255,255,0.3))",borderRadius:12}}/>) : (<div style={{fontSize:60,marginBottom:12,filter:"drop-shadow(0 0 20px rgba(255,255,255,0.3))",cursor:"pointer",userSelect:"none"}}>נ</div>); })()}
          <h1 style={{color:"#fff",fontSize:24,fontWeight:900,margin:"0 0 6px",letterSpacing:"-0.5px"}}>{companyName}</h1>
          <p style={{color:"rgba(255,255,255,0.6)",fontSize:14,margin:0}}>׳׳¢׳¨׳›׳× ׳ ׳™׳”׳•׳ ׳‘׳¨׳™׳›׳•׳×</p>
          {clientPlan.plan&&(
            <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12}}>
              <span style={{background:"rgba(255,255,255,0.2)",borderRadius:99,padding:"4px 14px",fontSize:12,fontWeight:800,color:"#fff"}}>{clientPlan.plan==="PRO"?"נ’ PRO":clientPlan.plan==="Basic"?"ג¡ Basic":"נ”¬ ׳ ׳™׳¡׳™׳•׳"}</span>
              <span style={{background:clientPlan.status==="׳₪׳¢׳™׳"?"rgba(46,125,50,0.5)":"rgba(198,40,40,0.5)",borderRadius:99,padding:"4px 14px",fontSize:12,fontWeight:800,color:"#fff"}}>{clientPlan.status==="׳₪׳¢׳™׳"?"ג… ׳₪׳¢׳™׳":"ג›” "+clientPlan.status}</span>
            </div>
          )}
        </div>
        <div style={{background:"#fff",borderRadius:24,padding:24,boxShadow:"0 20px 60px rgba(0,0,0,0.25)"}}>
          <h2 style={{fontSize:18,fontWeight:900,color:C.text,margin:"0 0 20px",textAlign:"center"}}>׳›׳ ׳™׳¡׳” ׳׳׳¢׳¨׳›׳×</h2>
          <div style={{marginBottom:12}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳©׳ ׳׳©׳×׳׳©</label><input value={loginUser} onChange={e=>setLoginUser(e.target.value)} placeholder="׳”׳›׳ ׳¡ ׳©׳ ׳׳©׳×׳׳©" style={inp} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/></div>
          <div style={{marginBottom:loginErr?12:20}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳¡׳™׳¡׳׳”</label><input type="password" value={loginPass} onChange={e=>setLoginPass(e.target.value)} placeholder="׳”׳›׳ ׳¡ ׳¡׳™׳¡׳׳”" style={inp} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/></div>
          {loginErr&&<div style={{background:"#ffebee",borderRadius:10,padding:"10px 14px",marginBottom:16,color:C.red,fontSize:13,fontWeight:700,textAlign:"center"}}>ג ן¸ {loginErr}</div>}
          <Press onClick={handleLogin} style={{padding:16,borderRadius:14,background:loginLoading?"#90caf9":`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:900,fontSize:16,textAlign:"center",boxShadow:loginLoading?"none":"0 6px 20px rgba(21,101,192,0.4)"}}>
            {actionLabel("login",{idle:"׳›׳ ׳™׳¡׳” ג†’",loading:"ג³ ׳׳×׳—׳‘׳¨...",success:"ג… ׳”׳×׳—׳‘׳¨׳×",error:"ג ן¸ ׳ ׳¡׳” ׳©׳•׳‘"})}
          </Press>
        </div>
        <InstallAppCard/>
      ...
      <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:90}}>
        ...
        <div style={{background:`linear-gradient(145deg,#0d47a1,${C.blue},${C.lightBlue})`,padding:"28px 20px 44px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
            <div>
              <p style={{color:"rgba(255,255,255,0.65)",fontSize:12,fontWeight:600,margin:"0 0 4px"}}>{fmtDate(dailyDate)} נ</p>
              <h1 style={{color:"#fff",fontSize:24,fontWeight:900,margin:0,lineHeight:1.1}}>׳©׳׳•׳, {user?.name}! {user?.icon}</h1>
              <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,margin:"4px 0 0"}}>{greeting || user?.welcomeMessage}</p>
              {clientPlan.plan&&(
                <div style={{display:"flex",gap:6,marginTop:8}}>
                  <span style={{background:"rgba(255,255,255,0.2)",borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:800,color:"#fff"}}>{clientPlan.plan==="PRO"?"נ’ PRO":clientPlan.plan==="Basic"?"ג¡ Basic":"נ”¬ ׳ ׳™׳¡׳™׳•׳"}</span>
                  <span style={{background:clientPlan.status==="׳₪׳¢׳™׳"?"rgba(46,125,50,0.4)":"rgba(198,40,40,0.4)",borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:800,color:"#fff"}}>{clientPlan.status==="׳₪׳¢׳™׳"?"ג… ׳₪׳¢׳™׳":"ג›” "+clientPlan.status}</span>
                </div>
              )}
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <div onPointerDown={()=>{logoLongPress.current=setTimeout(()=>{haptic("success");setShowSetup(true);},3000);}} onPointerUp={()=>clearTimeout(logoLongPress.current)} onPointerLeave={()=>clearTimeout(logoLongPress.current)} style={{fontSize:18,cursor:"pointer",userSelect:"none",padding:"4px 6px",color:"rgba(255,255,255,0.4)"}}>ג™ן¸</div>
              <Press onClick={handleLogout} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 12px",color:"rgba(255,255,255,0.8)",fontSize:12,fontWeight:700}}>׳™׳¦׳™׳׳”</Press>
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,position:"relative"}}>
            {[[`${done}/${dayTasks.length}`,"׳׳©׳™׳׳•׳×","נ“‹"],[`${dayTasks.length>0?Math.round((done/dayTasks.length)*100):0}%`,"׳”׳•׳©׳׳","ג…"],[workStart?workStart:"--:--","׳”׳×׳—׳׳”","ג±ן¸"]].map(([n,l,ic])=>(
        ...
        <div style={{margin:"-20px 16px 0",position:"relative",zIndex:10}}>
          ...
          <div style={{...card({marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}),padding:"14px 18px"}}>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4}}>׳©׳¢׳•׳ ׳¢׳‘׳•׳“׳”</div>
              <div style={{fontSize:16,fontWeight:800,color:workStart?C.blue:C.muted}}>{workStart?`ג–¶ ׳₪׳¢׳™׳ ׳-${workStart}`:"׳׳ ׳”׳×׳—׳™׳"}</div>
            </div>
        ...
        <div style={{padding:"16px 16px 0"}}>
          {pending.length>0&&!dismissed&&(
            <div style={{...card({background:"#fff8e1",border:"1px solid #ffe082",marginBottom:12,display:"flex",alignItems:"center",gap:10}),padding:"12px 16px"}}>
              <span style={{fontSize:18}}>ג ן¸</span>
              <div style={{flex:1}}><div style={{fontWeight:800,fontSize:13,color:C.orange}}>{pending.length} ׳“׳•׳—׳•׳× ׳׳׳×׳™׳ ׳™׳ ׳׳©׳׳™׳—׳”</div><div style={{fontSize:11,color:C.muted}}>׳©׳׳•׳¨׳™׳ ׳׳§׳•׳׳™׳× ג€” ׳׳—׳¥ ׳׳©׳׳™׳—׳”</div></div>
              <Press onClick={syncPendingReports} style={{background:C.orange,borderRadius:99,padding:"6px 12px",color:"#fff",fontWeight:800,fontSize:12}}>{actionLabel("syncPending",{idle:"׳©׳׳—",loading:"ג³ ׳©׳•׳׳—...",success:"ג… ׳ ׳©׳׳—",error:"ג ן¸ ׳ ׳¡׳” ׳©׳•׳‘"})}</Press>
              <Press onClick={()=>setDismissed(true)} style={{color:C.muted,fontSize:18,padding:"0 4px"}}>ג•</Press>
            </div>
          )}
          <Press onClick={openManualReport} disabled={isActionLoading("openManualReport")} style={{...card({marginBottom:16,display:"flex",alignItems:"center",gap:12,border:`2px dashed ${C.lightBlue}`,background:isActionLoading("openManualReport")?"#e3f2fd":"#f5f9ff",opacity:isActionLoading("openManualReport")?0.75:1}),padding:"14px 18px"}}>
            <div style={{width:40,height:40,borderRadius:12,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>נ“</div>
            <div><div style={{fontWeight:800,fontSize:15,color:C.blue}}>{isActionLoading("openManualReport")?"ג³ ׳₪׳•׳×׳— ׳“׳•׳—...":"+ ׳₪׳×׳— ׳“׳•׳— ׳—׳“׳©"}</div><div style={{fontSize:12,color:C.muted}}>׳“׳•׳— ׳™׳“׳ ׳™ ג€” ׳׳§׳•׳— ׳׳›׳ ׳”׳¨׳©׳™׳׳”</div></div>
          </Press>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              <h2 style={{fontSize:12,fontWeight:800,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",margin:0}}>׳¡׳™׳“׳•׳¨ ׳™׳•׳׳™</h2>
              <div style={{fontSize:13,fontWeight:800,color:C.blue,marginTop:2}}>׳™׳•׳ {dateDayName(dailyDate)}</div>
            </div>
          ...
          <div style={{marginBottom:12,position:"relative"}}>
            <input value={clientSearch} onChange={e=>setClientSearch(e.target.value)} placeholder="נ” ׳—׳₪׳© ׳׳§׳•׳— ׳׳›׳ ׳”׳™׳׳™׳..." style={{...inp,fontSize:13}}/>
            {clientSearch&&(
              <div style={{position:"absolute",top:"100%",right:0,left:0,background:"#fff",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,0.15)",zIndex:100,maxHeight:240,overflowY:"auto",border:`1px solid ${C.border}`,marginTop:4}}>
                {[...clients,...unassignedClients.filter(uc=>!clients.find(c=>c.name===uc.name))].filter(c=>c.name.toLowerCase().includes(clientSearch.toLowerCase())).map(c=>(
                  <Press key={c.name} onClick={()=>{ setForm({...blank(),client:c.name,reportDate:dailyDate,clientLocked:true}); setClientSearch(""); setScreen("form"); haptic(); }} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",borderBottom:`1px solid ${C.border}`,background:"#fff"}}>
                    <div style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:"#fff",flexShrink:0}}>{(c.poolType==="׳›׳׳•׳¨")?"נ§×":(c.poolType==="׳’׳׳™׳©׳”")?"נ":(c.poolType==="׳¡׳§׳™׳׳¨")?"נ”µ":"נ§‚"}</div>
                ...
                {[...clients,...unassignedClients].filter(c=>c.name.toLowerCase().includes(clientSearch.toLowerCase())).length===0&&<div style={{padding:"14px 16px",color:C.muted,fontSize:13}}>׳׳ ׳ ׳׳¦׳ ׳׳§׳•׳—</div>}
              </div>
            )}
          </div>
          {dayTasks.length===0&&<div style={{...card({textAlign:"center"}),padding:32}}><div style={{fontSize:40,marginBottom:8}}>נ“­</div><div style={{fontWeight:700,color:C.muted,fontSize:14}}>׳׳™׳ ׳׳§׳•׳—׳•׳× ׳׳×׳׳¨׳™׳ ׳–׳”</div></div>}
          {dayTasks.map((t,i)=>{
            ...
            const poolType = (clients.find(c=>c.name===t.client)||{}).poolType||"׳׳׳—";
            const poolIcon = poolType==="׳›׳׳•׳¨"?"נ§×":poolType==="׳’׳׳™׳©׳”"?"נ":poolType==="׳¡׳§׳™׳׳¨"?"נ”µ":"נ§‚";
            return (
              <div key={t.id} style={{...card({marginBottom:12,opacity:isDone?0.65:1,border:`2px solid ${needsAck?"#ff9800":isDone?"#c8e6c9":C.border}`,transition:"all 0.3s"})}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                      ...
                      <div>
                        ...
                        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginTop:2}}>
                          <span style={{fontSize:10,fontWeight:700,background:poolType==="׳›׳׳•׳¨"?"#e3f2fd":poolType==="׳’׳׳™׳©׳”"?"#e0f7fa":poolType==="׳¡׳§׳™׳׳¨"?"#e8eaf6":"#e8f5e9",color:poolType==="׳›׳׳•׳¨"?C.blue:poolType==="׳’׳׳™׳©׳”"?"#006064":poolType==="׳¡׳§׳™׳׳¨"?"#3949ab":C.green,borderRadius:99,padding:"2px 8px"}}>{poolType}</span>
                          {clientAddress(t.client)&&<span style={{fontSize:11,color:C.muted}}>נ“ {clientAddress(t.client)}</span>}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
                    <Badge label={isDone?"ג“ ׳‘׳•׳¦׳¢":"ג³ ׳׳׳×׳™׳"} col={isDone?C.green:C.orange}/>
                    {!isDone&&<Press onClick={()=>{setForm({...blank(),client:t.client,reportDate:dailyDate,clientLocked:true});setScreen("form");}} style={{padding:"8px 14px",borderRadius:10,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:800,fontSize:12,boxShadow:"0 3px 10px rgba(21,101,192,0.3)"}}>נ“ ׳“׳•׳—</Press>}
                  </div>
                </div>
                {needsAck&&(
                  <div style={{background:"#fff8e1",borderRadius:10,padding:"10px 12px",marginBottom:10,border:"1px solid #ffe082"}}>
                    <div style={{fontSize:12,fontWeight:800,color:"#e65100",marginBottom:4}}>נ”” {lastLog.note}</div>
                    <div style={{fontSize:10,color:"#bf6900",marginBottom:8}}>{lastLog.at} ֲ· {lastLog.by}</div>
                    <Press onClick={()=>{ackChange(t.id,logIdx);haptic("success");}} style={{padding:"8px 16px",borderRadius:99,background:"#e65100",color:"#fff",fontWeight:800,fontSize:12,display:"inline-block"}}>׳§׳™׳‘׳׳×׳™ ג“</Press>
                  </div>
                )}
                  ...
                  return (
                    <div style={{marginBottom:10}}>
                      <div style={{background:"#e3f2fd",borderRadius:10,padding:"8px 12px",marginBottom:6,display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontSize:12,fontWeight:700,color:C.blue}}>נ“ ׳׳“׳™׳“׳” ׳׳—׳¨׳•׳ ׳”:</span>
                        ...
                        {lr.phUp>0&&<span style={{fontSize:12,fontWeight:800,color:"#6a1b9a"}}>pH+: {lr.phUp} ׳›׳•׳¡׳•׳×</span>}
                        {lr.acidLiters>0&&<span style={{fontSize:12,fontWeight:800,color:C.red}}>׳—׳•׳׳¦׳”: {lr.acidLiters}L</span>}
                ...
                {supply&&!isDone&&(
                  <div style={{background:"#e3f2fd",borderRadius:10,padding:"8px 12px",marginBottom:10}}>
                    <div style={{fontSize:11,fontWeight:700,color:C.blue,marginBottom:4}}>נ“¦ ׳¦׳™׳•׳“ ׳ ׳“׳¨׳©:</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {supply.acid&&<span style={{background:C.white,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700,color:C.text,border:"1px solid "+C.border}}>נ§× ׳—׳•׳׳¦׳× ׳׳׳—</span>}
                      {supply.phUpSupply&&<span style={{background:C.white,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700,color:C.text,border:"1px solid "+C.border}}>נ“ˆ ׳׳¢׳׳” pH</span>}
                      {supply.saltPkg&&<span style={{background:C.white,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700,color:C.text,border:"1px solid "+C.border}}>נ§‚ ׳׳׳— ֳ—{supply.saltBags}</span>}
                    </div>
                  </div>
                )}
                {clientGateCode(t.client)&&(
                  <Press onClick={()=>{setShowGateCode(g=>({...g,[t.client]:!g[t.client]}));haptic();}} style={{display:"inline-flex",alignItems:"center",gap:6,marginBottom:8,padding:"6px 14px",background:showGateCode[t.client]?"#fff3e0":"#f0f4f8",borderRadius:99,border:`1px solid ${showGateCode[t.client]?"#ffb74d":C.border}`}}>
                    <span style={{fontSize:12}}>נ”‘</span>
                    <span style={{fontSize:12,fontWeight:800,color:showGateCode[t.client]?C.orange:C.muted}}>{showGateCode[t.client]?clientGateCode(t.client):"׳”׳¦׳’ ׳§׳•׳“ ׳©׳¢׳¨"}</span>
                  </Press>
                )}
                {!isDone&&(
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {clientAddress(t.client)&&<a href={wazeUrl(clientAddress(t.client))} target="_blank" rel="noreferrer" style={{padding:"10px 14px",borderRadius:12,background:"#e8f5e9",color:C.green,fontWeight:800,fontSize:13,textDecoration:"none",textAlign:"center",border:"2px solid #c8e6c9",display:"flex",alignItems:"center",gap:4}}>נ—÷ן¸ ׳ ׳•׳•׳˜</a>}
                    {clientPhone(t.client)&&<a href={`tel:${clientPhone(t.client)}`} style={{padding:"10px 14px",borderRadius:12,background:"#f3e5f5",color:"#6a1b9a",fontWeight:800,fontSize:13,textDecoration:"none",textAlign:"center",border:"2px solid #e1bee7",display:"flex",alignItems:"center",gap:4}}>נ“</a>}
                  </div>
                )}
                <div style={{marginTop:isDone?0:8}}>
                  <Press onClick={()=>{setOpIssueClient(t.client);setShowOperatorIssue(true);haptic();}} style={{padding:"8px 14px",borderRadius:12,background:"#fff8e1",color:C.orange,fontWeight:800,fontSize:12,border:"1px solid #ffe082",display:"inline-flex",alignItems:"center",gap:6}}>נ”§ ׳“׳•׳•׳— ׳×׳§׳׳”</Press>
                </div>
              </div>
            );
          })}
          {done===dayTasks.length&&dayTasks.length>0&&!clientSearch&&(
            <div style={{...card({textAlign:"center",background:"linear-gradient(135deg,#e8f5e9,#f1f8e9)"}),padding:28,border:"2px solid #c8e6c9"}}>
              <div style={{fontSize:44,marginBottom:8}}>נ‰</div>
              <div style={{fontWeight:900,fontSize:18,color:C.green,marginBottom:4}}>׳¡׳™׳™׳׳× ׳”׳›׳!</div>
        ...
        {navTab===1&&(
          <BottomSheet title="נ“‹ ׳׳©׳™׳׳•׳× ׳”׳™׳•׳" onClose={()=>setNavTab(0)}>
            {(()=>{
              const todayTasks = myTasks(dailyDate);
              if(todayTasks.length===0) return <div style={{textAlign:"center",padding:32,color:C.muted}}><div style={{fontSize:40,marginBottom:8}}>נ“­</div><div style={{fontWeight:700}}>׳׳™׳ ׳׳©׳™׳׳•׳× ׳׳”׳™׳•׳</div></div>;
              return todayTasks.map(t=>(
                <div key={t.id} style={{...card({marginBottom:10})}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    ...
                    <Badge label={t.status==="done"?"ג“ ׳‘׳•׳¦׳¢":"ג³ ׳׳׳×׳™׳"} col={t.status==="done"?C.green:C.orange}/>
                  </div>
                  {clientAddress(t.client)&&<div style={{fontSize:12,color:C.muted,marginBottom:6}}>נ“ {clientAddress(t.client)}</div>}
                  {(t.changeLog?.[t.changeLog.length-1]?.note)&&<div style={{background:"#fff8e1",borderRadius:8,padding:"6px 10px",fontSize:12,color:C.orange,fontWeight:600,marginBottom:8}}>נ“ {t.changeLog[t.changeLog.length-1].note}</div>}
                  {t.status!=="done"&&<Press onClick={()=>{setForm({...blank(),client:t.client,reportDate:dailyDate,clientLocked:true});setNavTab(0);setScreen("form");haptic();}} style={{padding:"8px 14px",borderRadius:10,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:800,fontSize:12,display:"inline-block"}}>נ“ ׳₪׳×׳— ׳“׳•׳—</Press>}
                </div>
              ));
            })()}
          </BottomSheet>
        ...
        {navTab===2&&(
          <BottomSheet title="נ“… ׳׳©׳™׳׳•׳× ׳¢׳×׳™׳“׳™׳•׳×" onClose={()=>setNavTab(0)}>
            {(()=>{
              ...
              if(futureTasks.length===0) return <div style={{textAlign:"center",padding:32,color:C.muted}}><div style={{fontSize:40,marginBottom:8}}>נ“­</div><div style={{fontWeight:700}}>׳׳™׳ ׳׳©׳™׳׳•׳× ׳¢׳×׳™׳“׳™׳•׳×</div></div>;
              const grouped = {};
              ...
              return Object.entries(grouped).map(([date, dts])=>(
                <div key={date} style={{marginBottom:20}}>
                  <div style={{fontSize:12,fontWeight:800,color:C.blue,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
                    <span>נ“… {fmtDate(date)}</span>
                    <span style={{background:"#e3f2fd",borderRadius:99,padding:"2px 10px",color:C.blue,fontSize:11}}>{dts.length} ׳׳©׳™׳׳•׳×</span>
                  </div>
                  {dts.map(t=>(
                    <div key={t.id} style={{...card({marginBottom:8})}}>
                      ...
                      {clientAddress(t.client)&&<div style={{fontSize:12,color:C.muted,marginBottom:6}}>נ“ {clientAddress(t.client)}</div>}
                      {(t.changeLog?.[t.changeLog.length-1]?.note)&&<div style={{background:"#fff8e1",borderRadius:8,padding:"6px 10px",fontSize:12,color:C.orange,fontWeight:600}}>נ“ {t.changeLog[t.changeLog.length-1].note}</div>}
                      {clientAddress(t.client)&&<a href={wazeUrl(clientAddress(t.client))} target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:8,padding:"6px 12px",background:"#e8f5e9",borderRadius:8,color:C.green,fontSize:12,fontWeight:700,textDecoration:"none"}}>נ—÷ן¸ ׳ ׳•׳•׳˜</a>}
                    </div>
                  ))}
                </div>
              ));
        ...
        {showOperatorIssue&&(
          <BottomSheet title="נ”§ ׳“׳•׳•׳— ׳¢׳ ׳×׳§׳׳”" onClose={()=>setShowOperatorIssue(false)}>
            <div>
              <div style={{marginBottom:12}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳׳§׳•׳—</label><div style={{...inp,color:C.blue,fontWeight:700}}>{opIssueClient}</div></div>
              <div style={{marginBottom:12}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳™׳׳•׳¨ ׳”׳×׳§׳׳”</label><textarea value={opIssueDesc} onChange={e=>setOpIssueDesc(e.target.value)} rows={3} placeholder="׳×׳׳¨ ׳׳× ׳”׳‘׳¢׳™׳”..." style={{...inp,resize:"none"}}/></div>
              <div style={{marginBottom:16}}>
                <label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:8}}>׳“׳—׳™׳₪׳•׳×</label>
                <div style={{display:"flex",gap:8}}>{["׳¨׳’׳™׳","׳“׳—׳•׳£","׳§׳¨׳™׳˜׳™"].map(p=>(<Press key={p} onClick={()=>setOpIssuePriority(p)} style={{flex:1,padding:"10px",borderRadius:10,textAlign:"center",fontSize:13,fontWeight:800,background:opIssuePriority===p?(p==="׳§׳¨׳™׳˜׳™"?C.red:p==="׳“׳—׳•׳£"?C.orange:C.blue):"#f0f4f8",color:opIssuePriority===p?"#fff":C.muted}}>{p}</Press>))}</div>
              </div>
              <Press onClick={async()=>{ if(!opIssueDesc.trim()){showToast("ג ן¸ ׳ ׳ ׳׳×׳׳¨ ׳׳× ׳”׳×׳§׳׳”");return;} await sheetCall("saveOperatorIssue",{operator:user?.name,client:opIssueClient,desc:opIssueDesc,priority:opIssuePriority,date:todayStr()}); setOpIssueDesc("");setOpIssuePriority("׳¨׳’׳™׳");setShowOperatorIssue(false); showToast("ג… ׳×׳§׳׳” ׳“׳•׳•׳—׳” ׳׳׳“׳׳™׳");haptic("success"); }} style={{padding:"14px",borderRadius:14,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:900,fontSize:15,textAlign:"center",boxShadow:"0 4px 14px rgba(21,101,192,0.3)"}}>׳©׳׳— ׳“׳™׳•׳•׳— ג†’</Press>
            </div>
          </BottomSheet>
        )}
        {/* QR ׳•׳©׳™׳—׳” ׳׳•׳¡׳×׳¨׳™׳ ׳–׳׳ ׳™׳× */}
  ...
  if(screen==="form") return (
    <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:100}}>
      ...
      <div style={{background:`linear-gradient(145deg,#0d47a1,${C.blue},${C.lightBlue})`,padding:"24px 20px 28px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative"}}>
          <div><p style={{color:"rgba(255,255,255,0.65)",fontSize:12,fontWeight:600,margin:"0 0 4px"}}>{form.clientLocked?form.client.split(" - ")[0]:"׳‘׳—׳¨ ׳׳§׳•׳—"}</p><h1 style={{color:"#fff",fontSize:22,fontWeight:900,margin:0}}>נ“ ׳“׳•׳— ׳˜׳™׳₪׳•׳</h1></div>
          <Press onClick={()=>setScreen("daily")} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 14px",color:"rgba(255,255,255,0.8)",fontSize:13,fontWeight:700}}>ג† ׳—׳–׳¨׳”</Press>
        </div>
      </div>
      <div style={{padding:"20px 16px 0"}}>
        <Sec icon="נ“‹" title="׳₪׳¨׳˜׳™׳">
          <div style={{...card(),marginBottom:12}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
              <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳׳¨׳™׳</label><input type="date" value={reportDate} onChange={e=>sf("reportDate",e.target.value)} style={inp}/></div>
              <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳׳₪׳¢׳™׳</label><div style={{...inp,color:C.blue,fontWeight:700,display:"flex",alignItems:"center",gap:6,cursor:"default"}}><span>{user?.icon}</span>{user?.name}</div></div>
            </div>
            <div>
              <label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳׳§׳•׳—</label>
              {form.clientLocked?(
                <div style={{...inp,color:C.blue,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"default"}}><span>נ {client}</span><span style={{fontSize:12,color:C.muted}}>נ”’</span></div>
              ):(
                <select value={client} onChange={e=>sf("client",e.target.value)} style={sel}>
                  <option value="">׳‘׳—׳¨ ׳׳§׳•׳—...</option>
                  {clients.length>0&&<optgroup label="׳׳§׳•׳—׳•׳× ׳§׳‘׳•׳¢׳™׳">{clients.map(c=><option key={c.name}>{c.name}</option>)}</optgroup>}
                  {freeClients.length>0&&<optgroup label="׳׳§׳•׳—׳•׳× ׳ ׳•׳¡׳₪׳™׳">{freeClients.map(c=><option key={c.name}>{c.name}</option>)}</optgroup>}
                </select>
              )}
              {client&&clientPhone(client)&&<a href={`tel:${clientPhone(client)}`} style={{display:"flex",alignItems:"center",gap:8,marginTop:8,padding:"10px 14px",background:"#e8f5e9",border:`1px solid #c8e6c9`,borderRadius:12,textDecoration:"none",color:C.green,fontSize:13,fontWeight:700}}><span>נ“</span><span>{client.split(" - ")[0]}</span><span style={{color:C.muted,fontSize:12,marginRight:"auto"}}>׳׳—׳¥ ׳׳—׳™׳•׳’</span></a>}
            </div>
          </div>
        </Sec>
        ...
        {form.adminReport&&(()=>{
          ...
          const isSalt = !poolType || poolType==="׳׳׳—" || poolType==="׳’׳׳™׳©׳”" || poolType==="׳¡׳§׳™׳׳¨";
          if(!isSalt) return null;
          return (
            <Sec icon="ג¡" title="׳׳׳§׳˜׳¨׳•׳“׳”">
              <div style={{...card()}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳“׳’׳</label><input value={elModel} onChange={e=>sf("elModel",e.target.value)} style={inp} placeholder="׳“׳’׳ ׳”׳׳›׳©׳™׳¨"/></div>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳¡׳¨׳™׳׳׳™</label><input value={elSerial} onChange={e=>sf("elSerial",e.target.value)} style={inp} placeholder="׳׳¡׳₪׳¨ ׳¡׳¨׳™׳׳׳™"/></div>
                </div>
                <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳׳¨׳™׳ ׳ ׳™׳§׳™׳•׳ ׳׳—׳¨׳•׳</label><input type="date" value={elDate} onChange={e=>sf("elDate",e.target.value)} style={inp}/></div>
                {calcNext(elDate,30)&&(()=>{
                  ...
                  const txt=d<0?`ג ן¸ ׳‘׳“׳™׳§׳” ׳‘׳׳™׳—׳•׳¨ ׳©׳ ${Math.abs(d)} ׳™׳׳™׳`:d<7?`ג° ׳‘׳“׳™׳§׳” ׳‘׳¢׳•׳“ ${d} ׳™׳׳™׳ (${fmtDate(calcNext(elDate,30))})`:`ג… ׳‘׳“׳™׳§׳” ׳”׳‘׳׳”: ${fmtDate(calcNext(elDate,30))} (${d} ׳™׳׳™׳)`;
        ...
        <Sec icon="נ”" title="׳‘׳“׳™׳§׳•׳× ׳׳¦׳‘">
          <ToggleField label="נ’§ ׳’׳•׳‘׳” ׳׳™׳" value={waterLevel} onChange={v=>sf("waterLevel",v)}/>
          <ToggleField label="נ”µ ׳¦׳׳™׳׳•׳×" value={clarity} onChange={v=>sf("clarity",v)}/>
          <ToggleField label="נ§´ ׳₪׳¡ ׳©׳•׳׳" value={fat} onChange={v=>sf("fat",v)}/>
          <ToggleField label="נ€ ׳–׳¨׳™׳׳”" value={flow} onChange={v=>sf("flow",v)}/>
        </Sec>

        <Sec icon="נ" title="׳׳¦׳‘ ׳‘׳¨׳™׳›׳”">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:poolStatus==="׳׳—׳¨"?12:0}}>
            {["׳׳׳•׳–׳ ׳×","׳׳—׳¨"].map(opt=>(
              <Press key={opt} onClick={()=>{sf("poolStatus",opt);haptic();}} style={{padding:14,borderRadius:14,textAlign:"center",fontWeight:800,fontSize:14,background:poolStatus===opt?(opt==="׳׳׳•׳–׳ ׳×"?"#e8f5e9":"#ffebee"):C.white,color:poolStatus===opt?(opt==="׳׳׳•׳–׳ ׳×"?C.green:C.red):C.muted,border:`2px solid ${poolStatus===opt?(opt==="׳׳׳•׳–׳ ׳×"?"#c8e6c9":"#ffcdd2"):C.border}`,boxShadow:"0 2px 8px rgba(0,0,0,0.04)",transition:"all 0.2s"}}>
                {opt==="׳׳׳•׳–׳ ׳×"?"ג… ׳׳׳•׳–׳ ׳×":"ג ן¸ ׳׳—׳¨"}
              </Press>
            ))}
          </div>
          <div style={{...card()}}>
            <textarea value={customStatusText} onChange={e=>sf("customStatusText",e.target.value)} rows={2} placeholder={poolStatus==="׳׳—׳¨"?"׳×׳׳¨ ׳׳× ׳”׳‘׳¢׳™׳”...":"׳”׳¢׳¨׳” ׳§׳¦׳¨׳” ׳¢׳ ׳׳¦׳‘ ׳”׳‘׳¨׳™׳›׳” (׳׳•׳₪׳¦׳™׳•׳ ׳׳™)..."} style={{...inp,resize:"none",marginBottom:poolStatus==="׳׳—׳¨"?10:0}}/>
            {poolStatus==="׳׳—׳¨"&&(
              <>
                <label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳”׳’׳‘׳׳× ׳©׳™׳׳•׳© ׳¢׳“</label>
        ...
        <Sec icon="נ“¦" title="׳¦׳™׳•׳“ ׳׳˜׳™׳₪׳•׳ ׳”׳‘׳">
          <div style={{...card()}}>
            <div style={{background:"#e3f2fd",borderRadius:10,padding:"8px 12px",marginBottom:12,display:"flex",gap:6,alignItems:"center"}}><span>נ”’</span><span style={{fontSize:11,fontWeight:700,color:C.blue}}>׳₪׳ ׳™׳׳™ ׳‘׳׳‘׳“ ג€” ׳׳ ׳ ׳©׳׳— ׳׳׳§׳•׳—</span></div>
            {[["acid",acid,"נ§× ׳—׳•׳׳¦׳× ׳׳׳—"],["phUpSupply",phUpSupply,"נ“ˆ ׳׳¢׳׳” pH"],["saltPkg",saltPkg,"נ§‚ ׳©׳§׳™ ׳׳׳—"]].map(([k,v,lbl])=>(
              <Press key={k} onClick={()=>{sf(k,!v);haptic();}} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{width:26,height:26,borderRadius:8,border:`2px solid ${v?C.blue:C.border}`,background:v?C.blue:C.white,display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s",flexShrink:0}}>{v&&<span style={{color:"#fff",fontSize:14}}>ג“</span>}</div>
                <span style={{fontSize:14,fontWeight:700,color:C.text}}>{lbl}</span>
              </Press>
            ))}
            {saltPkg&&(
              <div style={{paddingTop:10}}>
                <label style={{fontSize:13,fontWeight:700,color:C.text,display:"block",marginBottom:8}}>׳›׳׳•׳× ׳©׳§׳™׳:</label>
        ...
        <Sec icon="נ“·" title="׳×׳׳•׳ ׳•׳×">
          <div style={{...card()}}>
            ...
            <Press onClick={()=>fileRef.current?.click()} style={{padding:"12px",borderRadius:12,border:`2px dashed ${C.lightBlue}`,background:"#f5f9ff",color:C.blue,fontWeight:700,fontSize:13,textAlign:"center"}}>{photos.length>0?`+ ׳”׳•׳¡׳£ ׳¢׳•׳“ ׳×׳׳•׳ ׳•׳× (${photos.length} ׳ ׳‘׳—׳¨׳•)`:"נ“¸ ׳”׳•׳¡׳£ ׳×׳׳•׳ ׳•׳×"}</Press>
          </div>
        </Sec>

        <Sec icon="נ“" title="׳”׳¢׳¨׳•׳× ׳׳׳§׳•׳—">
          <textarea value={notes} onChange={e=>sf("notes",e.target.value)} rows={3} placeholder="׳”׳¢׳¨׳” ׳§׳¦׳¨׳” ׳©׳×׳™׳©׳׳— ׳‘׳•׳•׳׳˜׳¡׳׳₪..." style={{...inp,resize:"none",minHeight:80}}/>
        </Sec>
        ...
        {pending.length>0&&(
          <div style={{...card({background:"#fff8e1",border:`1px solid #ffe082`}),marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:13,fontWeight:700,color:C.orange}}>ג ן¸ {pending.length} ׳“׳•׳—׳•׳× ׳׳׳×׳™׳ ׳™׳ ׳׳©׳׳™׳—׳”</span>
            <Press onClick={syncPendingReports} style={{background:C.orange,borderRadius:99,padding:"6px 14px",color:"#fff",fontWeight:800,fontSize:12}}>{actionLabel("syncPending",{idle:"׳©׳׳— ׳”׳›׳",loading:"ג³ ׳©׳•׳׳—...",success:"ג… ׳ ׳©׳׳—",error:"ג ן¸ ׳ ׳¡׳” ׳©׳•׳‘"})}</Press>
          </div>
        )}
        <Press onClick={handleSubmit} disabled={!client||syncing||isActionLoading("submitReport")} style={{padding:"18px",borderRadius:16,background:actionStatus.submitReport==="success"?C.green:actionStatus.submitReport==="local"?C.orange:syncing||!client?"#90caf9":`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:900,fontSize:17,textAlign:"center",boxShadow:syncing||!client?"none":"0 8px 24px rgba(21,101,192,0.4)",marginBottom:8}}>{actionLabel("submitReport",{idle:"׳©׳׳— ׳“׳•׳— ג¡",loading:"ג³ ׳©׳•׳׳— ׳“׳•׳—...",success:"ג… ׳ ׳©׳׳—",local:"ג ן¸ ׳ ׳©׳׳¨ ׳׳§׳•׳׳™׳×",error:"ג ן¸ ׳©׳’׳™׳׳”"})}</Press>
        <Press onClick={()=>setScreen("daily")} style={{padding:"14px",borderRadius:14,border:`2px solid ${C.border}`,background:C.white,color:C.muted,fontWeight:700,fontSize:14,textAlign:"center"}}>ג† ׳‘׳™׳˜׳•׳</Press>
      </div>
      <Toast msg={toast.msg} visible={toast.visible}/>
    </div>
  );
  ...
  if(screen==="done") {
    ...
    return (
      <div dir="rtl" style={{minHeight:"100vh",background:`linear-gradient(145deg,#e3f2fd,${C.bg})`,fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
        ...
        <div style={{fontSize:72,marginBottom:16,animation:"pop 0.5s cubic-bezier(0.34,1.56,0.64,1)"}}>ג…</div>
        <h1 style={{fontSize:26,fontWeight:900,color:C.text,margin:"0 0 8px"}}>׳”׳“׳•׳— ׳ ׳©׳׳—!</h1>
        <p style={{color:C.lightBlue,fontSize:15,margin:"0 0 28px",fontWeight:600}}>׳”׳׳§׳•׳— ׳™׳§׳‘׳ ׳”׳•׳“׳¢׳× WhatsApp ׳¢׳›׳©׳™׳• נ’¬</p>
        {last&&(
          <div style={{...card({width:"100%",maxWidth:340,marginBottom:20,textAlign:"right"})}}>
            <div style={{fontSize:12,fontWeight:800,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:12}}>׳¡׳™׳›׳•׳ ׳”׳“׳•׳—</div>
            {[["׳׳§׳•׳—",last.client.split(" - ")[0]],["׳›׳׳•׳¨",`${last.chlorine} ppm`],["pH",last.ph],["׳׳׳—",`${last.salt} g/L`],["׳׳¦׳‘",last.poolStatus==="׳׳׳•׳–׳ ׳×"?"ג… ׳׳׳•׳–׳ ׳×":"ג ן¸ "+last.customStatusText]].map(([k,v])=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,width:"100%",maxWidth:340,marginBottom:10}}>
          <Press onClick={()=>{setForm(blank());setScreen("form");haptic();}} style={{padding:14,borderRadius:14,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:800,fontSize:14,textAlign:"center",boxShadow:"0 6px 20px rgba(21,101,192,0.35)"}}>+ ׳“׳•׳— ׳—׳“׳©</Press>
          <Press onClick={()=>setScreen("daily")} style={{padding:14,borderRadius:14,border:`2px solid ${C.border}`,background:C.white,color:C.blue,fontWeight:800,fontSize:14,textAlign:"center"}}>נ  ׳׳•׳— ׳™׳•׳׳™</Press>
        </div>
        {reports.length>0&&(
          <Press onClick={()=>{ const last=reports[reports.length-1]; setForm({...blank(),...last,clientLocked:true,reportDate:last.reportDate,client:last.client}); setEditingReport(last.id); setReports(r=>r.slice(0,-1)); setScreen("form"); haptic("medium"); showToast("גן¸ ׳¢׳¨׳•׳ ׳•׳”׳’׳© ׳׳—׳“׳©"); }} style={{padding:12,borderRadius:12,border:`2px solid ${C.orange}`,background:"#fff8e1",color:C.orange,fontWeight:800,fontSize:13,textAlign:"center",width:"100%",maxWidth:340}}>גן¸ ׳¢׳¨׳•׳ ׳“׳•׳— ׳׳—׳¨׳•׳</Press>
        )}
      </div>
    );
  }
    ...
    return (
      <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:30}}>
        ...
        <div style={{background:`linear-gradient(145deg,#0d47a1,${C.blue},${C.lightBlue})`,padding:"28px 20px 24px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",position:"relative"}}>
            <div>
              <p style={{color:"rgba(255,255,255,0.65)",fontSize:12,fontWeight:600,margin:"0 0 4px"}}>׳₪׳׳ ׳ ׳ ׳™׳”׳•׳ נ‘”</p>
              <h1 style={{color:"#fff",fontSize:22,fontWeight:900,margin:0}}>׳©׳׳•׳, {user?.name}</h1>
              {clientPlan.plan&&(
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <Press onClick={()=>{setAdminTab("daily");window.scrollTo(0,0);haptic();}} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 12px",color:"rgba(255,255,255,0.8)",fontSize:12,fontWeight:700}}>נ“‹</Press>
              <Press onClick={()=>{setForm({...blank(),adminReport:true});setScreen("form");haptic("medium");}} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 12px",color:"rgba(255,255,255,0.8)",fontSize:12,fontWeight:700}}>נ“ ׳“׳•׳—</Press>
              <Press onClick={handleLogout} style={{background:"rgba(255,255,255,0.15)",backdropFilter:"blur(8px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:12,padding:"8px 12px",color:"rgba(255,255,255,0.8)",fontSize:12,fontWeight:700}}>׳™׳¦׳™׳׳”</Press>
            </div>
          </div>
        </div>
        <div style={{background:C.white,padding:"8px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:6,overflowX:"auto",position:"sticky",top:0,zIndex:50,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
          {[["daily","נ“‹ ׳—׳׳•׳§׳× ׳¢׳‘׳•׳“׳”"],["adminreport","נ“ ׳“׳•׳— ׳™׳“׳ ׳™"],["progress","נ“ ׳”׳×׳§׳“׳׳•׳×"],["hours","ג±ן¸ ׳©׳¢׳•׳×"],["qr","נ“· QR"],["clients","נ‘¥ ׳׳§׳•׳—׳•׳×"],["reports","נ“„ ׳“׳•׳—׳•׳×"],["opissues","נ”§ ׳×׳§׳׳•׳× ׳׳₪׳¢׳™׳"],["supply","נ“¦ ׳—׳•׳׳¨׳™׳"],["users","נ‘₪ ׳׳©׳×׳׳©׳™׳"]].map(([t,lbl])=>(
        ...
        <div style={{padding:"20px 16px 0"}}>
          ...
          {adminTab==="adminreport"&&(
            <div>
              <div style={{...card({marginBottom:16,background:"#e3f2fd",border:`1px solid #90caf9`}),padding:"12px 16px",display:"flex",gap:10}}><span style={{fontSize:18}}>ג„¹ן¸</span><span style={{fontSize:12,color:C.blue,fontWeight:600}}>׳׳׳ ׳“׳•׳— ׳˜׳™׳₪׳•׳ ׳™׳“׳ ׳™ ג€” ׳׳›׳ ׳׳§׳•׳—</span></div>
              <div style={{marginBottom:14}}>
                <label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳‘׳—׳¨ ׳׳§׳•׳—</label>
                {form.client?(<div style={{...inp,display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"default"}}><span style={{color:C.blue,fontWeight:700}}>נ {form.client.split(" - ")[0]}</span><span onClick={()=>{sf("client","");setAdminClientSearch("");}} style={{color:C.muted,cursor:"pointer",fontSize:16}}>ג•</span></div>):(
                  <div style={{position:"relative"}}>
                    <input value={adminClientSearch} onChange={e=>setAdminClientSearch(e.target.value)} placeholder="נ” ׳—׳₪׳© ׳׳§׳•׳— ׳׳₪׳™ ׳©׳ ׳׳• ׳›׳×׳•׳‘׳×..." style={inp} autoComplete="off"/>
                    {adminClientSearch&&(
                      <div style={{position:"absolute",top:"100%",right:0,left:0,background:"#fff",borderRadius:12,boxShadow:"0 8px 24px rgba(0,0,0,0.15)",zIndex:100,maxHeight:260,overflowY:"auto",border:`1px solid ${C.border}`,marginTop:4}}>
                        {clients.filter(c=>`${c.name||""} ${c.address||""} ${c.phone||""}`.toLowerCase().includes(adminClientSearch.toLowerCase())).map(c=>(
                          <Press key={c.name} onClick={()=>{sf("client",c.name);setAdminClientSearch("");haptic();}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",borderBottom:`1px solid ${C.border}`,background:"#fff"}}>
                            <div style={{width:32,height:32,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:"#fff",flexShrink:0}}>נ</div>
                            <div><div style={{fontWeight:700,fontSize:13,color:C.text}}>{c.name.split(" - ")[0]}</div>{c.address&&<div style={{fontSize:11,color:C.muted}}>{c.address}</div>}</div>
                          </Press>
                        ))}
                        {clients.filter(c=>`${c.name||""} ${c.address||""} ${c.phone||""}`.toLowerCase().includes(adminClientSearch.toLowerCase())).length===0&&<div style={{padding:"14px 16px",color:C.muted,fontSize:13}}>׳׳ ׳ ׳׳¦׳ ׳׳§׳•׳—</div>}
                      </div>
                    )}
                  </div>
                )}
              </div>
              <Press onClick={()=>{ if(!form.client){showToast("ג ן¸ ׳‘׳—׳¨ ׳׳§׳•׳—");return;} setForm(f=>({...f,clientLocked:true,adminReport:true})); setScreen("form"); haptic("medium"); }} disabled={!form.client} style={{padding:"14px",borderRadius:14,background:form.client?`linear-gradient(135deg,${C.blue},${C.lightBlue})`:"#90caf9",color:"#fff",fontWeight:900,fontSize:15,textAlign:"center",boxShadow:form.client?"0 4px 14px rgba(21,101,192,0.3)":"none",marginBottom:8}}>נ“ ׳₪׳×׳— ׳“׳•׳— ׳׳׳“׳׳™׳</Press>
            </div>
          )}
          {adminTab==="daily"&&(
            <div>
              <div style={{...card({marginBottom:16})}}>
                <h3 style={{fontSize:14,fontWeight:800,color:C.text,margin:"0 0 14px"}}>{editTaskId?"גן¸ ׳¢׳¨׳™׳›׳× ׳׳©׳™׳׳”":"ג• ׳”׳•׳¡׳₪׳× ׳׳©׳™׳׳•׳×"}</h3>
                <div style={{marginBottom:10}}><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳׳¨׳™׳</label><input type="date" value={taskDate} onChange={e=>setTaskDate(e.target.value)} style={inp}/></div>
                {!editTaskId&&(
                  <div style={{marginBottom:10}}>
                    <label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳׳§׳•׳—׳•׳× <span style={{color:C.blue}}>({taskClients.length} ׳ ׳‘׳—׳¨׳•)</span></label>
                    <div style={{position:"relative",marginBottom:8}}><input value={taskClientSearch} onChange={e=>setTaskClientSearch(e.target.value)} placeholder="נ” ׳—׳₪׳© ׳•׳¡׳׳ ׳׳§׳•׳—׳•׳×..." style={inp} autoComplete="off"/></div>
                      ...
                      {clients.filter(c=>!taskClientSearch||c.name.toLowerCase().includes(taskClientSearch.toLowerCase())).map(c=>{
                        ...
                        return (
                          <Press key={c.name} onClick={()=>{ haptic(); setTaskClients(prev=>selected?prev.filter(x=>x.name!==c.name):[...prev,{name:c.name,note:""}]); }} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderBottom:`1px solid ${C.border}`,background:selected?"#e3f2fd":"transparent"}}>
                            <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${selected?C.blue:C.border}`,background:selected?C.blue:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{selected&&<span style={{color:"#fff",fontSize:13,fontWeight:900}}>ג“</span>}</div>
                    ...
                    {taskClients.length>0&&(
                      <div style={{marginTop:10}}>
                        <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:8}}>׳׳§׳•׳—׳•׳× ׳ ׳‘׳—׳¨׳™׳ ג€” ׳”׳•׳¡׳£ ׳”׳¢׳¨׳” ׳׳›׳ ׳׳—׳“:</div>
                        {taskClients.map((tc,i)=>(
                          <div key={tc.name} style={{background:C.white,borderRadius:12,padding:"10px 12px",marginBottom:8,border:`1px solid ${C.border}`}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                              <span style={{fontWeight:800,fontSize:13,color:C.blue}}>נ {tc.name.split(" - ")[0]}</span>
                              <Press onClick={()=>setTaskClients(prev=>prev.filter(x=>x.name!==tc.name))} style={{color:C.muted,fontSize:16,padding:"0 4px"}}>ג•</Press>
                            </div>
                            <input value={tc.note} onChange={e=>{ const updated=[...taskClients]; updated[i]={...tc,note:e.target.value}; setTaskClients(updated); }} placeholder="׳”׳¢׳¨׳” ׳¡׳₪׳¦׳™׳₪׳™׳× ׳׳׳§׳•׳— ׳–׳” (׳׳•׳₪׳¦׳™׳•׳ ׳׳™)..." style={{...inp,fontSize:12,padding:"8px 12px"}}/>
                          </div>
                        ))}
                        <Press onClick={()=>setTaskClients([])} style={{padding:"6px 14px",borderRadius:99,background:"#ffebee",color:C.red,fontWeight:700,fontSize:12,display:"inline-block"}}>׳ ׳§׳” ׳”׳›׳</Press>
                      </div>
                    )}
                  </div>
                )}
                {editTaskId&&(
                  <div style={{marginBottom:10}}>
                    <label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳׳§׳•׳—</label>
                    {taskClient?(<div style={{...inp,display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"default"}}><span style={{color:C.blue,fontWeight:700}}>נ {taskClient.split(" - ")[0]}</span><span onClick={()=>{setTaskClient("");setTaskClientSearch("");}} style={{color:C.muted,cursor:"pointer",fontSize:16}}>ג•</span></div>):(
                      <div style={{position:"relative"}}>
                        <input value={taskClientSearch} onChange={e=>setTaskClientSearch(e.target.value)} placeholder="נ” ׳—׳₪׳© ׳׳§׳•׳—..." style={inp} autoComplete="off"/>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <select defaultValue="" onChange={e=>{if(e.target.value&&!taskOps.includes(e.target.value)){setTaskOps([...taskOps,e.target.value]);e.target.value="";}}} style={sel}><option value="">+ ׳”׳•׳¡׳£ ׳׳₪׳¢׳™׳</option>{opNames.filter(n=>!taskOps.includes(n)).map(n=><option key={n}>{n}</option>)}</select>
                </div>
                <div style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳”׳¢׳¨׳” (׳×׳•׳₪׳™׳¢ ׳׳¦׳ ׳”׳׳₪׳¢׳™׳)</label><input value={taskNote} onChange={e=>setTaskNote(e.target.value)} placeholder="׳”׳¢׳¨׳” ׳׳•׳₪׳¦׳™׳•׳ ׳׳™׳×..." style={inp}/></div>
                <Press onClick={async()=>{
                  ...
                  if(editTaskId){
                    ...
                    const newTasksBatch = taskClients.map(tc=>({id:Date.now()+Math.floor(Math.random()*100000),date:taskDate.slice(0,10),client:tc.name,operators:[...taskOps],status:"pending",changeLog:[{at:nowStr(),note:tc.note||taskNote||"נ“‹ ׳׳©׳™׳׳” ׳—׳“׳©׳” ׳”׳•׳§׳¦׳×׳” ׳׳",by:user?.name,needsAck:true,ackedBy:[]}]}));
                    const newTasks = [...tasks, ...newTasksBatch];
                    setTasks(newTasks); setTaskClients([]); setTaskClientSearch(""); setTaskOps([]); setTaskNote("");
                    if(sheetId) await sheetCall("saveTasks",{tasks:newTasks});
                    setAction("saveTasks", "success", 1500);
                    showToast(`ג… ${newTasksBatch.length} ׳׳©׳™׳׳•׳× ׳ ׳•׳¦׳¨׳•`);
                    haptic("success");
                    ...
                    setTimeout(async () => {
                      ...
                      for(const opName of notifyOps) {
                        ...
                        const sent = await sendOneSignalToUser(`נ“‹ ׳׳©׳™׳׳•׳× ׳—׳“׳©׳•׳×`, `${clientList} ג€” ${fmtDate(notifyDate)}`, opUser?.username);
                        if(sent) sentCount++;
                      }
                      if (sentCount === notifyOps.length) showToast(`ג… ׳”׳”׳×׳¨׳׳•׳× ׳ ׳©׳׳—׳•`);
                      else if (missingCount) showToast(`ג ן¸ ׳—׳¡׳¨ ׳©׳ ׳׳©׳×׳׳© ׳-${missingCount} ׳׳₪׳¢׳™׳׳™׳`);
                      else showToast(`ג ן¸ ${sentCount}/${notifyOps.length} ׳”׳×׳¨׳׳•׳× ׳ ׳©׳׳—׳•`);
                    }, 0);
                  }
                  ...
                  {actionStatus.saveTasks==="loading"?"ג³ ׳©׳•׳׳¨ ׳•׳©׳•׳׳—...":actionStatus.saveTasks==="success"?"ג… ׳ ׳©׳׳¨ ׳•׳ ׳©׳׳—":actionStatus.saveTasks==="warning"?"ג ן¸ ׳ ׳©׳׳¨, ׳‘׳“׳•׳§ ׳”׳×׳¨׳׳•׳×":editTaskId?"נ’¾ ׳©׳׳•׳¨ ׳©׳™׳ ׳•׳™׳™׳":taskClients.length>1?`ג• ׳¦׳•׳¨ ${taskClients.length} ׳׳©׳™׳׳•׳×`:"ג• ׳”׳•׳¡׳£ ׳׳©׳™׳׳”"}
                </Press>
              </div>
              <h3 style={{fontSize:12,fontWeight:800,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",margin:"0 0 12px"}}>׳׳©׳™׳׳•׳× ג€” {fmtDate(taskDate)}</h3>
              {dayTasks.length===0&&<div style={{...card({textAlign:"center"}),padding:24,color:C.muted,fontSize:14}}>׳׳™׳ ׳׳©׳™׳׳•׳× ׳׳×׳׳¨׳™׳ ׳–׳”</div>}
              {dayTasks.map(t=>{
                const lastLog=t.changeLog?.[t.changeLog.length-1];
                return (
                  <div key={t.id} style={{...card({marginBottom:10})}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                      ...
                      <Badge label={t.status==="done"?"ג“ ׳‘׳•׳¦׳¢":"׳׳׳×׳™׳"} col={t.status==="done"?C.green:C.orange}/>
                    </div>
                    <div style={{display:"flex",gap:8}}>
                      <select defaultValue="" onChange={e=>{if(e.target.value){addOp(t.id,e.target.value);e.target.value="";}}} style={{...sel,flex:1,fontSize:12,padding:"7px 10px"}}><option value="">+ ׳”׳•׳¡׳£ ׳׳₪׳¢׳™׳</option>{opNames.filter(n=>!t.operators.includes(n)).map(n=><option key={n}>{n}</option>)}</select>
                      <Press onClick={()=>{setEditTaskId(t.id);setTaskClient(t.client);setTaskOps(t.operators);setTaskDate(t.date);window.scrollTo(0,0);}} style={{padding:"7px 14px",borderRadius:10,background:"#e3f2fd",color:C.blue,fontSize:12,fontWeight:700}}>גן¸</Press>
                      <Press onClick={async()=>{ if(!window.confirm("׳׳׳—׳•׳§?"))return; const deletedTask=t; const n=tasks.filter(x=>x.id!==t.id); setTasks(n); if(sheetId)await sheetCall("saveTasks",{tasks:n}); await sendNotificationToOperators(deletedTask.operators||[], "נ—‘ן¸ ׳׳©׳™׳׳” ׳ ׳׳—׳§׳”", `${deletedTask.client?.split(" - ")[0] || ""} ג€” ${fmtDate(deletedTask.date)}`); showToast("נ—‘ן¸ ׳׳©׳™׳׳” ׳ ׳׳—׳§׳”"); }} style={{padding:"7px 14px",borderRadius:10,background:"#ffebee",color:C.red,fontSize:12,fontWeight:700}}>נ—‘ן¸</Press>
                    </div>
                    {lastLog&&(
                      <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.border}`,fontSize:11,color:C.muted}}>
                        נ• {lastLog.at} ג€” {lastLog.note}
                        {lastLog.needsAck&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:4}}>{t.operators.map(op=>{ const acked=(lastLog.ackedBy||[]).includes(op); return <span key={op} style={{background:acked?"#e8f5e9":"#fff3e0",color:acked?C.green:C.orange,borderRadius:99,padding:"2px 8px",fontSize:10,fontWeight:700}}>{acked?"ג“":"ג³"} {op}</span>; })}</div>}
                      </div>
                    )}
                  </div>
                );
          ...
          {adminTab==="progress"&&(
            <div>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><label style={{fontSize:12,fontWeight:700,color:C.muted}}>׳×׳׳¨׳™׳:</label><input type="date" value={dailyDate} onChange={e=>setDailyDate(e.target.value)} style={{...inp,maxWidth:160,color:C.blue,border:`1px solid ${C.lightBlue}`,fontWeight:700}}/></div>
              {progressData.map(({op,total,done})=>(
                <div key={op.name} style={{...card({marginBottom:12})}}>
                  <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:total?14:0}}>
                    ...
                    <div style={{flex:1}}><div style={{fontWeight:800,fontSize:15,color:C.text}}>{op.name}</div><div style={{color:C.muted,fontSize:11,marginTop:2}}>{total===0?"׳׳™׳ ׳׳©׳™׳׳•׳× ׳”׳™׳•׳":`${done} ׳”׳•׳©׳׳׳• ֲ· ${total-done} ׳ ׳•׳×׳¨׳•`}</div></div>
                    <Badge label={`${done}/${total}`} col={done===total&&total>0?C.green:C.blue}/>
                  </div>
                  {total>0&&<PBar done={done} total={total}/>}
                  {tasks.filter(t=>t.date===dailyDate&&t.operators.includes(op.name)).map(t=>(<div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderTop:`1px solid ${C.border}`,marginTop:8}}><span style={{color:C.muted,fontSize:13}}>{t.client.split(" - ")[0]}</span><Badge label={t.status==="done"?"ג“ ׳‘׳•׳¦׳¢":"׳׳׳×׳™׳"} col={t.status==="done"?C.green:C.orange}/></div>))}
                </div>
              ))}
            </div>
          )}
          {adminTab==="qr"&&(
            <div>
              <div style={{...card({marginBottom:16,background:"#e3f2fd",border:`1px solid #90caf9`}),padding:"12px 16px",display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:20}}>ג„¹ן¸</span><div style={{fontSize:12,color:C.blue,fontWeight:600,lineHeight:1.6}}>׳׳›׳ ׳׳§׳•׳— ׳™׳© QR ׳™׳™׳—׳•׳“׳™. ׳”׳׳₪׳¢׳™׳ ׳¡׳•׳¨׳§ ׳׳•׳×׳• ג†’ ׳”׳“׳•׳— ׳ ׳₪׳×׳— ׳׳•׳˜׳•׳׳˜׳™׳×. ׳”׳“׳₪׳¡ ׳׳× ׳”-QR ׳•׳©׳™׳ ׳׳¦׳ ׳”׳׳§׳•׳—.</div></div>
              {clients.map(c=>(<div key={c.name} style={{...card({marginBottom:10}),display:"flex",alignItems:"center",gap:14}}><div style={{flex:1}}><div style={{fontWeight:800,fontSize:15,color:C.text,marginBottom:2}}>{c.name.split(" - ")[0]}</div><div style={{fontSize:12,color:C.muted}}>נ“ {c.address||c.name.split(" - ")[1]||""}</div></div><Press onClick={()=>setShowQRCode(showQRCode===c.name?null:c.name)} style={{padding:"8px 14px",borderRadius:10,background:showQRCode===c.name?"#e3f2fd":C.border,color:showQRCode===c.name?C.blue:C.muted,fontWeight:700,fontSize:12}}>{showQRCode===c.name?"׳¡׳’׳•׳¨":"נ“· QR"}</Press></div>))}
              {showQRCode&&(()=>{ const encoded = encodeURIComponent(showQRCode.split(" - ")[0]); const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encoded}&bgcolor=ffffff&color=1565c0&margin=10`; return (<div style={{...card({border:`2px solid ${C.lightBlue}`,textAlign:"center"}),padding:20,marginBottom:16}}><div style={{fontWeight:800,fontSize:15,color:C.text,marginBottom:12}}>{showQRCode.split(" - ")[0]}</div><img src={qrUrl} alt="QR" style={{width:180,height:180,borderRadius:12,marginBottom:12}}/><div style={{fontSize:11,color:C.muted,marginBottom:12}}>׳¡׳¨׳•׳§ ׳¢׳ ׳”׳׳₪׳׳™׳§׳¦׳™׳” ׳׳₪׳×׳™׳—׳× ׳“׳•׳—</div><a href={qrUrl} download={`qr-${showQRCode.split(" - ")[0]}.png`} target="_blank" rel="noreferrer"><Press style={{padding:"10px 20px",borderRadius:10,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:700,fontSize:13,display:"inline-block"}}>ג¬‡ן¸ ׳”׳•׳¨׳“ QR</Press></a></div>); })()}
            </div>
          )}
          {adminTab==="hours"&&(
            <div>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><label style={{fontSize:12,fontWeight:700,color:C.muted}}>׳×׳׳¨׳™׳:</label><input type="date" value={dailyDate} onChange={e=>setDailyDate(e.target.value)} style={{...inp,maxWidth:160,color:C.blue,border:`1px solid ${C.lightBlue}`,fontWeight:700}}/></div>
              {operatorUsers.map(op=>{ const logs=workLogs.filter(l=>l.operator===op.name&&l.date===dailyDate); const tot=logs.reduce((a,l)=>{const[h,m]=l.total.split(":").map(Number);return a+h*60+m;},0); const totStr=tot>0?`${Math.floor(tot/60)}:${String(tot%60).padStart(2,"0")}`:"ג€”"; return (<div key={op.name} style={{...card({marginBottom:12})}}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:logs.length?10:0}}><span style={{fontSize:28}}>{op.icon}</span><div style={{flex:1}}><div style={{fontWeight:800,fontSize:15,color:C.text}}>{op.name}</div><div style={{color:C.muted,fontSize:11,marginTop:2}}>{logs.length===0?"׳׳ ׳ ׳¨׳©׳׳” ׳¢׳‘׳•׳“׳”":`${logs.length} ׳¡׳©׳ ׳™׳ ֲ· ׳¡׳”"׳› ${totStr} ׳©׳¢׳•׳×`}</div></div>{tot>0&&<Badge label={`ג±ן¸ ${totStr}`} col={C.blue}/>}</div>{logs.map((l,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderTop:`1px solid ${C.border}`}}><span style={{color:C.muted,fontSize:12}}>נ• {l.start} ג€” {l.end}</span><span style={{color:C.blue,fontSize:12,fontWeight:800}}>{l.total} ׳©׳¢׳³</span></div>))}</div>); })}
            </div>
          )}
          {adminTab==="clients"&&(
            <div>
              <div style={{...card({marginBottom:16})}}>
                <h3 style={{fontSize:14,fontWeight:800,color:C.text,margin:"0 0 14px"}}>ג• ׳׳§׳•׳— ׳—׳“׳©</h3>
                <div style={{marginBottom:10}}><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳©׳ ׳׳§׳•׳— *</label><input value={newClient.name} onChange={e=>setNewClient(c=>({...c,name:e.target.value}))} placeholder="׳׳©׳₪׳—׳× ׳›׳”׳" style={inp}/></div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳˜׳׳₪׳•׳</label><input value={newClient.phone} onChange={e=>setNewClient(c=>({...c,phone:e.target.value}))} placeholder="05XXXXXXXX" style={inp} type="tel"/></div>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳›׳×׳•׳‘׳×</label><input value={newClient.address} onChange={e=>setNewClient(c=>({...c,address:e.target.value}))} placeholder="׳¨׳—׳•׳‘ ׳”׳™׳ 1" style={inp}/></div>
                </div>
                <Press onClick={async()=>{ if(!newClient.name.trim()){showToast("ג ן¸ ׳ ׳ ׳׳”׳–׳™׳ ׳©׳ ׳׳§׳•׳—");return;} const updated=[...clients,{name:newClient.name.trim(),phone:newClient.phone.trim(),address:newClient.address.trim(),qrUrl:""}]; setClients(updated); setNewClient({name:"",phone:"",address:""}); if(sheetId) await sheetCall("saveClients",{clients:updated}); showToast("ג… ׳׳§׳•׳— ׳ ׳•׳¡׳£"); haptic("success"); }} style={{padding:"13px",borderRadius:14,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:800,fontSize:14,textAlign:"center",boxShadow:`0 4px 14px rgba(21,101,192,0.3)`}}>ג• ׳”׳•׳¡׳£ ׳׳§׳•׳—</Press>
              </div>
              <h3 style={{fontSize:12,fontWeight:800,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",margin:"0 0 12px"}}>׳׳§׳•׳—׳•׳× ׳§׳™׳™׳׳™׳ ג€” {clients.length}</h3>
              {clients.map((c,i)=>(
                <div key={i} style={{...card({marginBottom:10})}}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:40,height:40,borderRadius:12,background:"#e3f2fd",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{(c.poolType==="׳›׳׳•׳¨")?"נ§×":(c.poolType==="׳’׳׳™׳©׳”")?"נ":(c.poolType==="׳¡׳§׳™׳׳¨")?"נ”µ":"נ§‚"}</div><div style={{flex:1}}><div style={{fontWeight:800,fontSize:14,color:C.text}}>{c.name.split(" - ")[0]}</div><div style={{fontSize:12,color:C.muted,marginTop:2}}>{c.phone} ֲ· {c.address}</div></div>{c.phone&&<a href={`tel:${c.phone}`} style={{color:C.blue,fontSize:18,textDecoration:"none"}}>נ“</a>}</div>
                  <div style={{display:"flex",gap:6,marginTop:10,flexWrap:"wrap"}}>{["׳׳׳—","׳›׳׳•׳¨","׳’׳׳™׳©׳”","׳¡׳§׳™׳׳¨"].map(pt=>(<Press key={pt} onClick={async()=>{ const updated=clients.map(x=>x.name===c.name?{...x,poolType:pt}:x); setClients(updated); await sheetCall("saveClientPoolType",{clientName:c.name,poolType:pt}); showToast(`ג… ${c.name.split(" - ")[0]} ג€” ${pt}`); haptic(); }} style={{padding:"5px 12px",borderRadius:99,fontSize:11,fontWeight:800,background:(c.poolType||"׳׳׳—")===pt?C.blue:"#f0f4f8",color:(c.poolType||"׳׳׳—")===pt?"#fff":C.muted}}>{pt}</Press>))}</div>
                </div>
              ))}
            </div>
          )}
          {adminTab==="reports"&&(
            <div>
              <div style={{...card({marginBottom:14})}}>
                <div style={{marginBottom:10}}><input value={reportFilter} onChange={e=>setReportFilter(e.target.value)} placeholder="נ” ׳—׳₪׳© ׳׳₪׳™ ׳׳§׳•׳— ׳׳• ׳׳₪׳¢׳™׳..." style={{...inp,marginBottom:0}}/></div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳׳¨׳™׳</label><input type="date" value={reportDateFilter} onChange={e=>setReportDateFilter(e.target.value)} style={inp}/></div>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>מפעיל</label><select value={reportOperatorFilter} onChange={e=>setReportOperatorFilter(e.target.value)} style={sel}><option value="">כל המפעילים</option>{reportOperatorOptions.map(n=><option key={n} value={n}>{n}</option>)}</select></div>
                  <div style={{display:"flex",alignItems:"flex-end"}}><Press onClick={async()=>{ showToast("ג³ ׳˜׳•׳¢׳ ׳“׳•׳—׳•׳×..."); const res = await sheetCall("getReports"); if(res?.reports?.length){setSheetReports(res.reports);showToast(`ג… ${res.reports.length} ׳“׳•׳—׳•׳× ׳ ׳˜׳¢׳ ׳•`);}else{showToast("ג ן¸ ׳׳ ׳ ׳׳¦׳׳• ׳“׳•׳—׳•׳×");} }} style={{width:"100%",padding:"12px",borderRadius:14,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:800,fontSize:13,textAlign:"center"}}>נ”„ ׳˜׳¢׳ ׳׳’׳™׳׳™׳•׳</Press></div>
                </div>
              </div>
              {(()=>{
                ...
                const filtered = unique.reverse().filter(r=>{ const matchText = !reportFilter || r.client?.includes(reportFilter) || r.operator?.includes(reportFilter); const matchDate = !reportDateFilter || r.reportDate===reportDateFilter; const matchOperator = !reportOperatorFilter || r.operator===reportOperatorFilter; return matchText && matchDate && matchOperator; });
                if(filtered.length===0) return <div style={{...card({textAlign:"center"}),padding:32,color:C.muted,fontSize:14}}>׳׳™׳ ׳“׳•׳—׳•׳× ג€” ׳׳—׳¥ "׳˜׳¢׳ ׳׳’׳™׳׳™׳•׳"</div>;
                return filtered.map((r,i)=>(
                  <div key={i} style={{...card({marginBottom:12})}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}><div><div style={{fontWeight:800,fontSize:15,color:C.text}}>{r.client?.split(" - ")[0]}</div><div style={{fontSize:12,color:C.muted,marginTop:2}}>נ‘₪ {r.operator} ֲ· נ“… {fmtDate(r.reportDate)}</div></div><Badge label={r.poolStatus==="׳׳׳•׳–׳ ׳×"?"ג… ׳׳׳•׳–׳ ׳×":"ג ן¸ ׳׳—׳¨"} col={r.poolStatus==="׳׳׳•׳–׳ ׳×"?C.green:C.orange}/></div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:6}}>{[["׳›׳׳•׳¨",`${r.chlorine} ppm`,"#e3f2fd","#1565c0"],["pH",r.ph,"#f3e5f5","#6a1b9a"],["׳׳׳—",`${r.salt} g/L`,"#e8f5e9","#1b5e20"]].map(([k,v,bg,col])=>(<div key={k} style={{background:bg,borderRadius:10,padding:"8px",textAlign:"center"}}><div style={{fontSize:10,fontWeight:700,color:"#90a4ae",marginBottom:2}}>{k}</div><div style={{fontSize:14,fontWeight:900,color:col}}>{v}</div></div>))}</div>
                    {r.notes&&<div style={{background:"#f5f9ff",borderRadius:10,padding:"8px 12px",fontSize:12,color:C.muted}}>נ“ {r.notes}</div>}
                    {r.supplyLabel&&<div style={{marginTop:8,fontSize:11,color:C.blue,fontWeight:700}}>נ“¦ {r.supplyLabel}</div>}
                  </div>
                ));
              })()}
            </div>
          ...
          {adminTab==="opissues"&&(
            <div>
              <Press onClick={async()=>{ const res=await sheetCall("getOperatorIssues"); if(res?.issues) setOperatorIssues(res.issues); showToast(`ג… ${res?.issues?.length||0} ׳×׳§׳׳•׳×`); }} style={{...card({marginBottom:14,background:"#e3f2fd",display:"flex",alignItems:"center",gap:10}),padding:"12px 16px"}}><span style={{fontSize:16}}>נ”„</span><span style={{fontWeight:700,fontSize:13,color:C.blue}}>׳˜׳¢׳ ׳×׳§׳׳•׳× ׳׳₪׳¢׳™׳</span></Press>
              {operatorIssues.length===0&&<div style={{...card({textAlign:"center"}),padding:32,color:C.muted}}>׳׳—׳¥ ׳˜׳¢׳ ׳׳¨׳׳•׳× ׳×׳§׳׳•׳×</div>}
              {operatorIssues.map((iss,i)=>{ const [id,operator,client,desc,priority,status,response,date]=iss; const priColor=priority==="׳§׳¨׳™׳˜׳™"?C.red:priority==="׳“׳—׳•׳£"?C.orange:C.blue; return (<div key={i} style={{...card({marginBottom:12,border:`2px solid ${priColor}22`})}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><div><div style={{fontWeight:800,fontSize:14,color:C.text}}>{client?.split(" - ")[0]}</div><div style={{fontSize:12,color:C.muted}}>נ‘₪ {operator} ֲ· נ“… {date}</div></div><div style={{display:"flex",gap:5}}><span style={{background:priColor+"18",color:priColor,borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:800}}>{priority}</span><span style={{background:status==="׳˜׳•׳₪׳"?"#e8f5e9":"#fff8e1",color:status==="׳˜׳•׳₪׳"?C.green:C.orange,borderRadius:99,padding:"3px 10px",fontSize:11,fontWeight:800}}>{status}</span></div></div><div style={{fontSize:13,color:"#546e7a",marginBottom:10,lineHeight:1.5}}>{desc}</div>{response&&<div style={{background:"#e8f5e9",borderRadius:8,padding:"8px 12px",fontSize:12,color:C.green,fontWeight:700,marginBottom:8}}>ג… ׳×׳’׳•׳‘׳× ׳׳“׳׳™׳: {response}</div>}<div style={{display:"flex",gap:8}}>{["׳‘׳˜׳™׳₪׳•׳","׳˜׳•׳₪׳"].map(s=>(<Press key={s} onClick={async()=>{ const updated=[...operatorIssues]; updated[i]=[...iss]; updated[i][5]=s; setOperatorIssues(updated); await sheetCall("updateOperatorIssue",{rowIndex:i+1,status:s}); showToast(`ג… ׳¢׳•׳“׳›׳ ׳-${s}`);haptic("success"); }} style={{padding:"7px 14px",borderRadius:99,fontSize:12,fontWeight:800,background:status===s?"#e8f5e9":"#f0f4f8",color:status===s?C.green:C.muted}}>{s}</Press>))}</div></div>); })}
            </div>
          )}
          {adminTab==="supply"&&(
            <div>
              <div style={{...card({marginBottom:14})}}>
                <div style={{fontWeight:800,fontSize:13,color:C.text,marginBottom:12}}>נ” ׳—׳™׳₪׳•׳© ׳—׳•׳׳¨׳™׳ ׳©׳¡׳•׳₪׳§׳•</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳׳¨׳™׳</label><input type="date" value={supplySearch.date} onChange={e=>setSupplySearch(s=>({...s,date:e.target.value}))} style={inp}/></div>
                  <div><label style={{fontSize:11,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳¡׳•׳’ ׳—׳•׳׳¨</label><select value={supplySearch.type} onChange={e=>setSupplySearch(s=>({...s,type:e.target.value}))} style={sel}><option value="">׳”׳›׳</option><option>׳›׳׳•׳¨ TAB</option><option>HTH</option><option>׳׳¢׳׳” pH</option><option>׳—׳•׳׳¦׳× ׳׳׳—</option><option>׳׳׳—</option></select></div>
                </div>
              </div>
              {(()=>{ const allRep=[...sheetReports,...reports]; const seen=new Set(); const filtered=allRep.filter(r=>{ if(seen.has(r.id))return false; seen.add(r.id); if(supplySearch.date&&r.reportDate!==supplySearch.date)return false; const hasSupply=r.chlora>0||r.hth>0||r.phUp>0||r.acidLiters>0||r.supplyLabel; if(!hasSupply)return false; if(supplySearch.type){const t=supplySearch.type;if(t==="׳›׳׳•׳¨ TAB"&&!(r.chlora>0))return false;if(t==="HTH"&&!(r.hth>0))return false;if(t==="׳׳¢׳׳” pH"&&!(r.phUp>0))return false;if(t==="׳—׳•׳׳¦׳× ׳׳׳—"&&!(r.acidLiters>0))return false;if(t==="׳׳׳—"&&!r.supplyLabel?.includes("׳׳׳—"))return false;} return true; }).sort((a,b)=>b.reportDate?.localeCompare(a.reportDate)); if(filtered.length===0)return <div style={{...card({textAlign:"center"}),padding:32,color:C.muted}}>׳׳™׳ ׳×׳•׳¦׳׳•׳× ג€” ׳׳—׳¥ "׳˜׳¢׳ ׳׳’׳™׳׳™׳•׳" ׳‘׳˜׳׳‘ ׳“׳•׳—׳•׳×</div>; return filtered.map((r,i)=>(<div key={i} style={{...card({marginBottom:10})}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div><div style={{fontWeight:800,fontSize:14,color:C.text}}>{r.client?.split(" - ")[0]}</div><div style={{fontSize:12,color:C.muted}}>נ‘₪ {r.operator} ֲ· נ“… {fmtDate(r.reportDate)}</div></div></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{r.chlora>0&&<span style={{background:"#fff3e0",color:C.orange,borderRadius:99,padding:"4px 12px",fontSize:12,fontWeight:700}}>TAB: {r.chlora}</span>}{r.hth>0&&<span style={{background:"#e8eaf6",color:"#283593",borderRadius:99,padding:"4px 12px",fontSize:12,fontWeight:700}}>HTH: {r.hth} cups</span>}{r.phUp>0&&<span style={{background:"#f3e5f5",color:"#6a1b9a",borderRadius:99,padding:"4px 12px",fontSize:12,fontWeight:700}}>pH+: {r.phUp} ׳›׳•׳¡׳•׳×</span>}{r.acidLiters>0&&<span style={{background:"#ffebee",color:C.red,borderRadius:99,padding:"4px 12px",fontSize:12,fontWeight:700}}>׳—׳•׳׳¦׳”: {r.acidLiters}L</span>}{r.supplyLabel&&<span style={{background:"#e8f5e9",color:C.green,borderRadius:99,padding:"4px 12px",fontSize:12,fontWeight:700}}>{r.supplyLabel}</span>}</div></div>)); })()}
            </div>
          )}
          {adminTab==="users"&&(
            <div>
              {allUsers.map(u=>(<div key={u.username} style={{...card({marginBottom:10,display:"flex",alignItems:"center",gap:12})}}><span style={{fontSize:30}}>{u.icon}</span><div style={{flex:1}}><div style={{fontWeight:800,fontSize:15,color:C.text}}>{u.name}</div><div style={{color:C.muted,fontSize:12,marginTop:2}}>{u.username} ֲ· {u.phone}</div><div style={{color:"#b0bec5",fontSize:11,marginTop:2}}>"{u.welcomeMessage}"</div></div><Badge label={u.role==="admin"?"׳׳ ׳”׳":"׳׳₪׳¢׳™׳"} col={u.role==="admin"?C.orange:C.blue}/></div>))}
              <div style={{marginTop:24}}><Press onClick={()=>setShowReportIssue(true)} style={{...card({background:"#fff8e1",border:"1px solid #ffe082",display:"flex",alignItems:"center",gap:12}),padding:"14px 16px"}}><span style={{fontSize:22}}>נ”§</span><div><div style={{fontWeight:800,fontSize:14,color:C.orange}}>׳“׳•׳•׳— ׳¢׳ ׳×׳§׳׳”</div><div style={{fontSize:12,color:C.muted}}>׳©׳׳— ׳“׳™׳•׳•׳— ׳™׳©׳™׳¨׳•׳× ׳׳׳₪׳×׳—</div></div></Press></div>
            </div>
          )}
        </div>
        {showReportIssue&&(
          <BottomSheet title="נ”§ ׳“׳•׳•׳— ׳¢׳ ׳×׳§׳׳”" onClose={()=>setShowReportIssue(false)}>
            <div>
              <div style={{marginBottom:12}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:6}}>׳×׳™׳׳•׳¨ ׳”׳‘׳¢׳™׳”</label><textarea value={issueDesc} onChange={e=>setIssueDesc(e.target.value)} rows={4} placeholder="׳×׳׳¨ ׳׳× ׳”׳‘׳¢׳™׳” ׳‘׳₪׳™׳¨׳•׳˜..." style={{...inp,resize:"none"}}/></div>
              <div style={{marginBottom:16}}><label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:8}}>׳¢׳“׳™׳₪׳•׳×</label><div style={{display:"flex",gap:8}}>{["׳¨׳’׳™׳","׳“׳—׳•׳£","׳§׳¨׳™׳˜׳™"].map(p=>(<Press key={p} onClick={()=>setIssuePriority(p)} style={{flex:1,padding:"10px",borderRadius:10,textAlign:"center",fontSize:13,fontWeight:800,background:issuePriority===p?(p==="׳§׳¨׳™׳˜׳™"?C.red:p==="׳“׳—׳•׳£"?C.orange:C.blue):"#f0f4f8",color:issuePriority===p?"#fff":C.muted}}>{p}</Press>))}</div></div>
              <Press onClick={async()=>{ if(!issueDesc.trim()){showToast("ג ן¸ ׳ ׳ ׳׳”׳–׳™׳ ׳×׳™׳׳•׳¨");return;} setSyncing(true); const company=getCompany(); await mgmtCall("saveMgmtIssue",{issue:[Date.now(), company.name||"׳׳ ׳™׳“׳•׳¢", todayStr(), issueDesc.trim(), issuePriority, "׳₪׳×׳•׳—", "", ""]}); setSyncing(false); setIssueDesc(""); setIssuePriority("׳¨׳’׳™׳"); setShowReportIssue(false); showToast("ג… ׳”׳“׳™׳•׳•׳— ׳ ׳©׳׳—!"); haptic("success"); }} style={{padding:"14px",borderRadius:14,background:`linear-gradient(135deg,${C.blue},${C.lightBlue})`,color:"#fff",fontWeight:900,fontSize:15,textAlign:"center",boxShadow:"0 4px 14px rgba(21,101,192,0.3)"}}>{syncing?"ג³ ׳©׳•׳׳—...":"׳©׳׳— ׳“׳™׳•׳•׳— ג†’"}</Press>
            </div>
          </BottomSheet>
        )}
        <Toast msg={toast.msg} visible={toast.visible}/>
