...
function doPost(e) {
  try {
    ...
    if (action === "validateLicense") {
      ...
      return json({ valid:true, company:String(row[1]), sheetId:String(row[2]), plan:String(row[3]||"PRO"), status, expiry, adminEmail:String(row[6]||"") });
    }
    ...
    if (action === "saveReport") {
      ...
      sheet.appendRow([r.reportDate, r.operator, r.client, r.chlorine, r.ph, r.salt,
        ...
        r.restrictedUntil, r.notes, r.chlora||0, r.hth||0, r.phUp||0, r.acidLiters||0]);
...
function setupClientSheet(clientSS) {
  ...
  if(!s) {
    ...
    s.appendRow(["תאריך","מפעיל","לקוח","כלור","pH","מלח","גובה_מים","צלילות","פס_שומן","זרימה","דגם_אלקטרודה","סריאלי_אלקטרודה","תאריך_ניקיון","תאריך_ניקיון_הבא","ציוד_נדרש","מצב_בריכה","פירוט_מצב","הגבלה_עד","הערות","chlora","hth","phUp","acidLiters"]);
  } else {
    ensureColumns(s, ["תאריך","מפעיל","לקוח","כלור","pH","מלח","גובה_מים","צלילות","פס_שומן","זרימה","דגם_אלקטרודה","סריאלי_אלקטרודה","תאריך_ניקיון","תאריך_ניקיון_הבא","ציוד_נדרש","מצב_בריכה","פירוט_מצב","הגבלה_עד","הערות","chlora","hth","phUp","acidLiters"]);
  }
...
function sendOneSignalToUser_(data) {
  const APP_ID = "dc1af269-2502-41a4-89d5-a3aa8d5be956";
  const REST_API_KEY = "os_v2_app_3qnpe2jfaja2jcovuovi2w7jk2vfqo2tc53exgepoubu3rvztgfwdmec7xlbcmrsavyh5gazzltxo77bqxkdlv6tgvyp3twhr32ryea";
...
function setupSheet() {
  ...
  if(reportSheet) {
    ...
    const neededR = ["chlora","hth","phUp","acidLiters"];
...
const SHEET_ID = "17jNBWSAkW17zfz4o2gY3wOsERa3_NAgSZ3b9HPkNspk";
