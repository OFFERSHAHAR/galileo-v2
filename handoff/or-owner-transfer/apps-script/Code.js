  ...
  function doPost(e) {
    try {
      ...
      if (
        action === "installAdvancedSheetsControl" ||
        action === "ensureAdvancedSheetsControl"
      ) {
        return json(installAdvancedSheetsControl(data.sheetId || SHEET_ID));
      }
  ...
  function setupClientSheet(clientSS) {
    ...
    ensureAdvancedSheetsControl_(clientSS, { installTrigger:false });
  ...
  function onOpen(e) {
    SpreadsheetApp.getUi()
      .createMenu("גלילאו")
      .addItem("פקד סינון מתקדם", "showAdvancedFilterSidebar")
      .addItem("נקה סינון", "clearAdvancedSheetFilter")
      .addToUi();
  }

  function installAdvancedFilterMenu() {
    onOpen();
    return { success:true };
  }

  function advancedFilterControlSheetName_() {
    return "פקד_סינון";
  }

  function showAdvancedFilterSidebar() {
    const html = HtmlService.createHtmlOutput(advancedFilterSidebarHtml_())
      .setTitle("פקד סינון מתקדם");
    SpreadsheetApp.getUi().showSidebar(html);
  }

  function advancedFilterSpreadsheet_(spreadsheetId) {
    if (spreadsheetId) return SpreadsheetApp.openById(String(spreadsheetId));
    return SpreadsheetApp.getActiveSpreadsheet() ||
      SpreadsheetApp.openById("1NthErqOJOFHJ482q3zg2daFX9SGCFeByXjdoZxvV-no");
  }

  function advancedFilterNormalize_(value) {
  ...
  function advancedFilterDateKey_(value) {
  ...
  function advancedFilterDateNumber_(value) {
    const key = advancedFilterDateKey_(value);
    return key ? Number(key.replace(/-/g, "")) : null;
  }

  function advancedFilterColumn_(headers, aliases) {
    const normalize = (value) => advancedFilterNormalize_(value).replace(/[\s_\-]/g, "");
    const wanted = aliases.map(normalize);
    for (let i = 0; i < headers.length; i++) {
      if (wanted.includes(normalize(headers[i]))) return i;
    }
    return -1;
  }

  function advancedFilterHeaderInfo_(sheet) {
    const headerRowIndex = findHeaderRowIndex_(sheet, [
      "תאריך",
      "לקוח",
      "שם_לקוח",
      "שם לקוח",
    ...
    const headers = sheet.getRange(headerRow, 1, 1, lastCol).getValues()[0]
      .map(h => String(h || "").trim());
    return { headerRow, lastCol, headers };
  }

  function getAdvancedFilterContext(spreadsheetId) {
    const ss = advancedFilterSpreadsheet_(spreadsheetId);
    const active = ss.getActiveSheet();
    const controlName = advancedFilterControlSheetName_();
    const sheets = ss.getSheets().filter(sheet => sheet.getName() !== controlName).map(sheet => {
      const info = advancedFilterHeaderInfo_(sheet);
      return {
        name: sheet.getName(),
        headerRow: info.headerRow,
        headers: info.headers.filter(Boolean),
        rows: Math.max(sheet.getLastRow() - info.headerRow, 0)
      };
    });
    return {
      success:true,
      spreadsheetName: ss.getName(),
      activeSheet: active ? active.getName() : "",
      sheets
    };
  }

  function advancedFilterHideRows_(sheet, rowNumbers) {
  ...
  function clearAdvancedSheetFilter(sheetName, spreadsheetId) {
    const ss = advancedFilterSpreadsheet_(spreadsheetId);
    const sheet = sheetName ? ss.getSheetByName(String(sheetName)) : ss.getActiveSheet();
    if (!sheet) return { success:false, error:"הגיליון לא נמצא" };
    const filter = sheet.getFilter();
    if (filter) filter.remove();
    const maxRows = Math.max(sheet.getMaxRows(), 1);
    sheet.showRows(1, maxRows);
    return { success:true, sheetName:sheet.getName(), shown:maxRows };
  }

  function runAdvancedSheetFilter(criteria) {
    const options = criteria || {};
    const ss = advancedFilterSpreadsheet_(options.spreadsheetId || "");
    const sheet = options.sheetName ? ss.getSheetByName(String(options.sheetName)) : ss.getActiveSheet();
    if (!sheet) return { success:false, error:"הגיליון לא נמצא" };

    const info = advancedFilterHeaderInfo_(sheet);
    const lastRow = sheet.getLastRow();
    const dataRows = Math.max(lastRow - info.headerRow, 0);
    clearAdvancedSheetFilter(sheet.getName(), options.spreadsheetId || "");
    if (!dataRows) {
      return { success:true, sheetName:sheet.getName(), matched:0, hidden:0, headerRow:info.headerRow };
    }

    const headers = info.headers;
    const indexes = {
      date: advancedFilterColumn_(headers, ["תאריך", "תאריך_דוח", "reportDate", "date"]),
      client: advancedFilterColumn_(headers, ["לקוח", "שם_לקוח", "שם לקוח", "שם הלקוח", "client", "name"]),
      operator: advancedFilterColumn_(headers, ["מפעיל", "מפעילים", "operator", "operators"]),
      status: advancedFilterColumn_(headers, ["סטטוס", "מצב", "status"])
    };
    const fromDate = advancedFilterDateNumber_(options.fromDate || "");
    const toDate = advancedFilterDateNumber_(options.toDate || "");
    if ((fromDate || toDate) && indexes.date < 0) {
      return { success:false, error:"לא נמצאה עמודת תאריך בגיליון", sheetName:sheet.getName() };
    }

    const needles = {
      client: advancedFilterNormalize_(options.client || ""),
      operator: advancedFilterNormalize_(options.operator || ""),
      status: advancedFilterNormalize_(options.status || ""),
      freeText: advancedFilterNormalize_(options.freeText || "")
    };
    const missingColumns = [
      needles.client && indexes.client < 0 ? "לקוח" : "",
      needles.operator && indexes.operator < 0 ? "מפעיל" : "",
      needles.status && indexes.status < 0 ? "סטטוס או מצב" : ""
    ].filter(Boolean);
    if (missingColumns.length) {
      return {
        success:false,
        error:"לא נמצאה עמודת " + missingColumns.join(", ") + " בגיליון",
    ...
    values.forEach((row, offset) => {
      ...
      if (ok && (fromDate || toDate)) {
        const rowDate = advancedFilterDateNumber_(row[indexes.date]);
        ok = rowDate !== null && (!fromDate || rowDate >= fromDate) && (!toDate || rowDate <= toDate);
      }
      if (ok && needles.client && indexes.client >= 0) {
        ok = advancedFilterNormalize_(row[indexes.client]).includes(needles.client);
      }
      if (ok && needles.operator && indexes.operator >= 0) {
        ok = advancedFilterNormalize_(row[indexes.operator]).includes(needles.operator);
      }
      if (ok && needles.status && indexes.status >= 0) {
        ok = advancedFilterNormalize_(row[indexes.status]).includes(needles.status);
      }
      if (ok && needles.freeText) {
        ok = advancedFilterNormalize_(row.join(" ")).includes(needles.freeText);
      }
      if (ok) {
        matched++;
      } else {
        hiddenRows.push(info.headerRow + 1 + offset);
      }
    });

    advancedFilterHideRows_(sheet, hiddenRows);
    sheet.setActiveRange(sheet.getRange(info.headerRow, 1, Math.max(dataRows + 1, 1), Math.min(info.lastCol, 1)));
    return {
      success:true,
      sheetName:sheet.getName(),
      matched,
  ...
  function installAdvancedSheetsControl(sheetId) {
    const ss = advancedFilterSpreadsheet_(sheetId || "");
    return ensureAdvancedSheetsControl_(ss, { installTrigger:true });
  }

  function ensureAdvancedSheetsControl_(ss, options) {
    const settings = options || {};
    const controlName = advancedFilterControlSheetName_();
    let control = ss.getSheetByName(controlName);
    const existingValues = control ? control.getRange("B3:B9").getValues().map(row => row[0]) : [];
    if (!control) control = ss.insertSheet(controlName, 0);

    const sheetNames = ss.getSheets()
      .map(sheet => sheet.getName())
      .filter(name => name !== controlName);
    const defaultSheet = sheetNames.includes(String(existingValues[0] || "")) ?
      String(existingValues[0] || "") :
      (sheetNames.includes("דוחות") ? "דוחות" : (sheetNames[0] || ""));

    control.clear();
    if (control.setRightToLeft) control.setRightToLeft(true);
    control.getRange("A1:B1").merge();
    control.getRange("A1")
      .setValue("פקד סינון מתקדם")
    ...
    control.getRange("A3:B9").setValues([
      ["גיליון לסינון", defaultSheet],
      ["מתאריך", existingValues[1] || ""],
      ["עד תאריך", existingValues[2] || ""],
      ["לקוח", existingValues[3] || ""],
      ["מפעיל", existingValues[4] || ""],
      ["סטטוס או מצב", existingValues[5] || ""],
      ["טקסט חופשי", existingValues[6] || ""]
    ]);
    control.getRange("A11:B12").setValues([
      ["סנן עכשיו", false],
      ["נקה סינון", false]
    ]);
    control.getRange("A14:B14").setValues([["תוצאה", "מוכן"]]);

    control.getRange("A3:A14")
      .setFontWeight("bold")
      .setBackground("#e8eef8")
      .setFontColor("#172033");
    control.getRange("B3:B14")
      .setBackground("#ffffff")
      .setBorder(true, true, true, true, true, true, "#cbd5e1", SpreadsheetApp.BorderStyle.SOLID);
    control.getRange("A11:B12").setBackground("#ecfdf5");
    control.getRange("B11:B12").insertCheckboxes();
    control.getRange("B4:B5").setNumberFormat("yyyy-mm-dd");
    ...
    control.getRange("B11").setNote("סמן כאן כדי להפעיל סינון לפי הערכים למעלה.");
    control.getRange("B12").setNote("סמן כאן כדי להציג מחדש את כל השורות בגיליון שנבחר.");
    control.getRange("A16:B18").setValues([
      ["איך משתמשים", "ממלאים תנאים בתאים הלבנים"],
      ["הפעלה", "מסמנים סנן עכשיו"],
      ["ניקוי", "מסמנים נקה סינון"]
    ]);
    control.getRange("A16:A18").setFontWeight("bold").setBackground("#fff7ed");
    control.getRange("B16:B18").setBackground("#fffaf0");

    if (settings.installTrigger) ensureAdvancedFilterOnEditTrigger_(ss);
    return {
      success:true,
      sheetName:controlName,
      spreadsheetName:ss.getName(),
      spreadsheetId:ss.getId(),
      installedTrigger:!!settings.installTrigger,
      targetSheets:sheetNames
    };
  }

  function ensureAdvancedFilterOnEditTrigger_(ss) {
    const spreadsheetId = ss.getId();
    const handler = "advancedFilterOnEdit";
  ...
  function advancedFilterCriteriaFromControl_(ss) {
    const controlName = advancedFilterControlSheetName_();
    const control = ss.getSheetByName(controlName);
    if (!control) throw new Error("טאב הפקד לא נמצא");
    const values = control.getRange("B3:B9").getValues().map(row => row[0]);
    const firstDataSheet = ss.getSheets()
      .map(sheet => sheet.getName())
      .filter(name => name !== controlName)[0] || "";
    return {
      spreadsheetId:ss.getId(),
      sheetName:String(values[0] || firstDataSheet || ""),
      fromDate:values[1],
      toDate:values[2],
      client:values[3],
      operator:values[4],
  ...
  function advancedFilterOnEdit(e) {
    if (!e || !e.range) return;
    const range = e.range;
    const control = range.getSheet();
    if (!control || control.getName() !== advancedFilterControlSheetName_()) return;
    const cell = range.getA1Notation();
    if (cell !== "B11" && cell !== "B12") return;
    const checked = range.getValue() === true || String(e.value || "").toUpperCase() === "TRUE";
    if (!checked) return;

    try {
      const ss = e.source || SpreadsheetApp.getActiveSpreadsheet();
      const criteria = advancedFilterCriteriaFromControl_(ss);
      const result = cell === "B11"
        ? runAdvancedSheetFilter(criteria)
        : clearAdvancedSheetFilter(criteria.sheetName, criteria.spreadsheetId);
      if (!result || !result.success) {
        control.getRange("B14").setValue((result && result.error) || "הפעולה נכשלה");
        return;
      }
      const message = cell === "B11"
        ? `נמצאו ${result.matched} מתוך ${result.total} שורות`
        : `הסינון נוקה בגיליון ${result.sheetName}`;
      control.getRange("B14").setValue(message);
    } catch (err) {
      control.getRange("B14").setValue(err && err.message ? err.message : String(err));
    } finally {
      range.setValue(false);
    }
  }

  function advancedFilterSidebarHtml_() {
    return `<!doctype html>
<html dir="rtl">
<head>
  <base target="_top">
  <style>
    body{font-family:Arial,sans-serif;margin:0;background:#f6f8fc;color:#172033}
    .wrap{padding:14px}
    h2{font-size:18px;margin:0 0 12px;font-weight:800}
    label{display:block;font-size:12px;font-weight:800;margin:10px 0 5px;color:#44546a}
    input,select{box-sizing:border-box;width:100%;height:38px;border:1px solid #cbd5e1;border-radius:10px;background:#fff;padding:0 10px;font-size:13px}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:14px}
    button{height:40px;border:0;border-radius:12px;font-weight:800;cursor:pointer}
    .primary{background:#2563eb;color:#fff}
    .soft{background:#e8eef8;color:#1d4ed8}
    .danger{background:#fff1f2;color:#be123c}
    .status{margin-top:12px;padding:10px;border-radius:12px;background:#fff;border:1px solid #e2e8f0;font-size:12px;line-height:1.5;min-height:22px}
    .error{border-color:#fecdd3;background:#fff1f2;color:#be123c}
    .hint{font-size:11px;color:#64748b;line-height:1.5;margin-top:10px}
  </style>
</head>
<body>
  <div class="wrap">
    <h2>פקד סינון מתקדם</h2>
    <label>גיליון</label>
    ...
    <label>לקוח</label>
    <input id="client" type="text" placeholder="שם לקוח">
    <label>מפעיל</label>
    <input id="operator" type="text" placeholder="שם מפעיל">
    <label>סטטוס או מצב</label>
    <input id="statusValue" type="text" placeholder="בוצע / ממתין / לא בוצע">
    <label>טקסט חופשי</label>
    <input id="freeText" type="text" placeholder="חיפוש בכל השורה">
    <div class="actions">
      <button class="primary" onclick="applyFilter()">סנן</button>
      <button class="soft" onclick="clearFilter()">נקה</button>
    </div>
    <button class="danger" style="width:100%;margin-top:8px" onclick="loadContext()">רענן רשימת גיליונות</button>
    <div id="statusBox" class="status">טוען...</div>
    <div class="hint">הפקד עובד על הנתונים הקיימים בגיליון. הוא מסתיר שורות שלא עומדות בתנאים, ולא מוסיף עמודות.</div>
  </div>
  <script>
    ...
    function loadContext(){
      ...
      google.script.run
        .withSuccessHandler(function(ctx){
          var select=el('sheetName');
          select.innerHTML='';
          (ctx.sheets||[]).forEach(function(sheet){
            var option=document.createElement('option');
            option.value=sheet.name;
            option.textContent=sheet.name+' ('+sheet.rows+' שורות)';
            if(sheet.name===ctx.activeSheet) option.selected=true;
            select.appendChild(option);
          });
          setStatus('מוכן לסינון בגיליון: '+(ctx.activeSheet||''));
        })
        .withFailureHandler(function(err){setStatus((err&&err.message)||String(err),true);})
        .getAdvancedFilterContext();
    }
    function applyFilter(){
      setStatus('מסנן...');
      google.script.run
        .withSuccessHandler(function(res){
          if(!res || !res.success){setStatus((res&&res.error)||'הסינון נכשל',true);return;}
          setStatus('נמצאו '+res.matched+' מתוך '+res.total+' שורות. הוסתרו '+res.hidden+' שורות.');
        })
        .withFailureHandler(function(err){setStatus((err&&err.message)||String(err),true);})
        .runAdvancedSheetFilter(payload());
    }
    function clearFilter(){
      setStatus('מנקה...');
      google.script.run
        .withSuccessHandler(function(res){
          if(!res || !res.success){setStatus((res&&res.error)||'ניקוי נכשל',true);return;}
          setStatus('הסינון נוקה בגיליון: '+res.sheetName);
        })
        .withFailureHandler(function(err){setStatus((err&&err.message)||String(err),true);})
        .clearAdvancedSheetFilter(el('sheetName').value);
    }
    loadContext();
  </script>
</body>
